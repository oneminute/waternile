  ...
  orm.initialize = function initialize(options, done) {
    try {
      ...
      if (_.keys(modelMap).length > 0) {
        throw new Error(
          'A Waterline ORM instance cannot be initialized more than once. To reset the ORM, create a new instance of it by running `new Waterline()`.'
        );
      }
      ...
      if (!_.isUndefined(options.connections)) {
        // Sanity check
        assert(
          _.isUndefined(options.datastores),
          'Attempted to provide backwards-compatibility for `connections`, but `datastores` was ALSO defined!  This should never happen.'
        );
      ...
      if (
        _.isUndefined(options.adapters) ||
        !_.isPlainObject(options.adapters)
      ) {
      ...
      if (
        _.isUndefined(options.datastores) ||
        !_.isPlainObject(options.datastores)
      ) {
      ...
      if (areAnyModelsUsingAtRestEncryption) {
        ...
        var parsedNodeMajorAndMinorVersion =
          process.version.match(RX_NODE_MAJOR_DOT_MINOR) &&
          +process.version.match(RX_NODE_MAJOR_DOT_MINOR)[1];
        var MIN_NODE_VERSION = 6;
        var isNativeCryptoFullyCapable =
          parsedNodeMajorAndMinorVersion >= MIN_NODE_VERSION;
        if (!isNativeCryptoFullyCapable) {
          throw new Error(
            "Current installed node version's native `crypto` module is not fully capable of the necessary functionality for encrypting/decrypting data at rest with Waterline.  To use this feature, please upgrade to Node v" +
              MIN_NODE_VERSION +
              " or above, flush your node_modules, run npm install, and then try again.  Otherwise, if you cannot upgrade Node.js, please remove the `encrypt` property from your models' attributes."
          );
        }
      ...
      _.each(wmds, function (wmd) {
            ...
            if (attrDef.encrypt !== undefined) {
              if (!_.isBoolean(attrDef.encrypt)) {
                throw flaverr({
                  ...
                  attrName: attrName,
                  message:
                    'If set, `encrypt` must be either `true` or `false`.',
                });
              } //•
              ...
              if (attrDef.encrypt === true) {
                ...
                if (attrDef.defaultsTo !== undefined) {
                  ...
                  throw flaverr({
                    ...
                    attrName: attrName,
                    whyNotCompatible:
                      'with an attribute that also specifies a `defaultsTo`.  ' +
          ...
          switch (err.code) {
            case 'E_INVALID_ENCRYPT':
              throw flaverr(
                {
                  message:
                    'Invalid usage of `encrypt` in the definition for `' +
                    modelDef.identity +
                    "` model's " +
                    '`' +
                    err.attrName +
              ...
              throw flaverr(
                {
                  message:
                    ...
                    '`' +
                    err.attrName +
                    '` attribute.  At-rest encryption (`encrypt: true`) cannot be used ' +
        ...
        if (modelDef.dataEncryptionKeys !== undefined) {
          if (
            !_.isObject(modelDef.dataEncryptionKeys) ||
            _.isArray(modelDef.dataEncryptionKeys) ||
            _.isFunction(modelDef.dataEncryptionKeys)
          ) {
            throw flaverr({
              message:
                'In the definition for the `' +
                modelDef.identity +
                '` model, the `dataEncryptionKeys` model setting ' +
            ...
            _.each(modelDef.dataEncryptionKeys, function (dek, dekId) {
              if (!dek || !_.isString(dek)) {
                throw flaverr({
                  ...
                  dekId: dekId,
                  message:
                    'Must be a cryptographically random, 32 byte string.',
                });
              } //•
              ...
              if (!dekId.match(/^[a-z\$]([a-z0-9])*$/i)) {
                throw flaverr({
                  ...
                  dekId: dekId,
                  message:
                    'Please make sure the ids of all of your data encryption keys begin with a letter and do not contain any special characters.',
                });
              } //•
              ...
              if (areAnyModelsUsingAtRestEncryption) {
                try {
                  EA(undefined, {
                    keys: modelDef.dataEncryptionKeys,
                    keyId: dekId,
                  }).encryptAttribute(
                    undefined,
                    'test-value-purely-for-validation'
                  );
                } catch (err) {
                  throw flaverr(
                    {
                ...
                throw flaverr(
                  {
                    message:
                      'In the definition for the `' +
                      modelDef.identity +
                      '` model, one of the data encryption keys (`dataEncryptionKeys.' +
                      err.dekId +
                      '`)  is invalid.\n' +
                      'Details:\n' +
                      '  ' +
        ...
        if (isThisModelUsingAtRestEncryption) {
          if (
            ...
            !modelDef.dataEncryptionKeys.default
          ) {
            throw flaverr({
              message:
      ...
      _.each(wmds, function (wmd) {
        ...
        if (modelDef.archiveModelIdentity === false) {
          ...
          return done(
            new Error(
              'Invalid `archiveModelIdentity` setting.  A model cannot be its own archive!  But model `' +
                modelDef.identity +
                "` has `archiveModelIdentity: '" +
                modelDef.archiveModelIdentity +
                "'`."
            )
          );
        } else if (
          !modelDef.archiveModelIdentity ||
          !_.isString(modelDef.archiveModelIdentity)
        ) {
          return done(
            new Error(
              'Invalid `archiveModelIdentity` setting.  If set, expecting either `false` (to disable .archive() altogether) or the identity of a registered model (e.g. "archive"), but instead got: ' +
                util.inspect(options.defaults.archiveModelIdentity, {
                  depth: null,
                })
            )
          );
        ...
        if (modelDef.archiveModelIdentity !== false) {
          if (
            !_.contains(
              Object.keys(archiversInfoByArchiveIdentity),
              modelDef.archiveModelIdentity
            )
          ) {
          ...
          archiversInfoByArchiveIdentity[
            modelDef.archiveModelIdentity
          ].archivers.push(modelDef);
        } //ﬁ
      }); //∞
      ...
      if (
        _.contains(
          Object.keys(archiversInfoByArchiveIdentity),
          DEFAULT_ARCHIVE_MODEL_IDENTITY
        )
      ) {
        ...
        var existingDefaultArchiveWmd = _.find(wmds, function (wmd) {
          return wmd.prototype.identity === DEFAULT_ARCHIVE_MODEL_IDENTITY;
        });
        if (!existingDefaultArchiveWmd) {
          var defaultArchiversInfo =
            archiversInfoByArchiveIdentity[DEFAULT_ARCHIVE_MODEL_IDENTITY];
      ...
      _.each(
        archiversInfoByArchiveIdentity,
        function (archiversInfo, archiveIdentity) {
          var archiveWmd = _.find(wmds, function (wmd) {
            return wmd.prototype.identity === archiveIdentity;
          });
          if (!archiveWmd) {
            throw new Error(
              "Invalid `archiveModelIdentity` setting.  A model declares `archiveModelIdentity: '" +
                archiveIdentity +
                "'`, but there's no other model actually registered with that identity to use as an archive!"
            );
          }
          ...
          var namesOfMissingAttrs = _.difference(
            EXPECTED_ATTR_NAMES,
            actualAttrNames
          );
          ...
          try {
            if (namesOfMissingAttrs.length > 0) {
              throw flaverr({
                ...
                because:
                  'it is missing ' +
                  namesOfMissingAttrs.length +
                  ' mandatory attribute' +
                  (namesOfMissingAttrs.length === 1 ? '' : 's') +
                  ': ' +
                  namesOfMissingAttrs +
                  '.',
              });
            ...
            if (archiveWmd.prototype.primaryKey !== 'id') {
              throw flaverr({
                code: 'E_INVALID_ARCHIVE_MODEL',
                because:
                  'it is using an attribute other than `id` as its logical primary key attribute.',
              });
            } //•
            ...
            if (_.any(EXPECTED_ATTR_NAMES, { encrypt: true })) {
              throw flaverr({
                code: 'E_INVALID_ARCHIVE_MODEL',
                because:
                  "it is using at-rest encryption on one of its mandatory attributes, when it shouldn't be.",
              });
            } //•
            ...
            switch (err.code) {
              case 'E_INVALID_ARCHIVE_MODEL':
                throw new Error(
                  'The `' +
                    archiveIdentity +
                    '` model cannot be used as a custom archive, because ' +
                    err.because +
                    '\n' +
                    'Please adjust this custom archive model accordingly, or otherwise switch to a different ' +
                    'model as your custom archive.  (For reference, this `' +
                    archiveIdentity +
                    '` model this is currently ' +
                    'configured as the custom archive model for ' +
                    archiversInfo.archivers.length +
                    ' other ' +
                    'model' +
                    (archiversInfo.archivers.length === 1 ? '' : 's') +
                    ': ' +
                    _.pluck(archiversInfo.archivers, 'identity') +
                    '.'
                );
              default:
                throw err;
    ...
    async.each(
      _.keys(datastoreMap),
      function (datastoreName, next) {
        var datastore = datastoreMap[datastoreName];

        if (_.isFunction(datastore.adapter.registerConnection)) {
          return next(
            new Error(
              'The adapter for datastore `' +
                datastoreName +
                '` is invalid: the `registerConnection` method must be renamed to `registerDatastore`.'
            )
          );
        }
        ...
        try {
          ...
          _.each(modelIdentities, function (modelIdentity) {
            ...
            assert(
              WLModel.tableName === tableName,
              'Expecting `WLModel.tableName === tableName`. (Please open an issue: http://sailsjs.com/bugs)'
            );
            assert(
              WLModel.identity === modelIdentity,
              'Expecting `WLModel.identity === modelIdentity`. (Please open an issue: http://sailsjs.com/bugs)'
            );
            assert(
              WLModel.primaryKey && _.isString(WLModel.primaryKey),
              "How flabbergasting!  Expecting truthy string in `WLModel.primaryKey`, but got something else. (If you're seeing this, there's probably a bug in Waterline.  Please open an issue: http://sailsjs.com/bugs)"
            );
            assert(
              WLModel.schema && _.isObject(WLModel.schema),
              'Expecting truthy string in `WLModel.schema`, but got something else. (Please open an issue: http://sailsjs.com/bugs)'
            );
            assert(
              WLModel.classType && _.isString(WLModel.classType),
              'Expecting truthy string in `WLModel.classType`, but got something else. (Please open an issue: http://github.com/gaithoben/sails-arangojs/issues)'
            );
            assert(
              WLModel.schemaValidation && _.isString(WLModel.schemaValidation),
              'Expecting truthy string in `WLModel.schemaValidation`, but got something else. (Please open an issue: http://github.com/gaithoben/sails-arangojs/issues)'
            );
            assert(
              WLModel.indexes && _.isArray(WLModel.indexes),
              'Expecting truthy Array in `WLModel.indexes`, but got something else. (Please open an issue: http://github.com/gaithoben/sails-arangojs/issues)'
            );
            assert(
              WLModel.tenantType && _.isArray(WLModel.tenantType),
              'Expecting truthy Array in `WLModel.tenantType`, but got something else. (Please open an issue: http://github.com/gaithoben/sails-arangojs/issues)'
            );
            assert(
              WLModel.additionalProperties &&
                _.isBoolean(WLModel.additionalProperties),
              'Expecting truthy Boolean in `WLModel.additionalProperties`, but got something else. (Please open an issue: http://github.com/gaithoben/sails-arangojs/issues)'
            );

            if (WLModel.classType === 'Edge') {
              usedSchemas[tableName] = {
                indexes: WLModel.indexes,
                classType: WLModel.classType,
                edgeDefinition: WLModel.edgeDefinition,
                primaryKey: WLModel.primaryKey,
                definition: WLModel.schema,
                tableName: tableName,
                identity: modelIdentity,
                datastore: WLModel.datastore,
                tenantType: WLModel.tenantType,
                schemaValidation: WLModel.schemaValidation,
                additionalProperties: WLModel.additionalProperties,
                globalId: WLModel.globalId,
                ModelObjectConstructor: WLModel.ModelObjectConstructor,
                keyProps: WLModel.keyProps,
                cache: WLModel.cache,
              };
            } else {
              usedSchemas[tableName] = {
          ...
          datastore.adapter.registerDatastore(
            datastore.config,
            usedSchemas,
            next
          );
...
module.exports.start = function (options, done) {
  ...
  if (!_.isFunction(done)) {
    throw new Error(
      'Please provide a valid callback function as the 2nd argument to `Waterline.start()`.  (Instead, got: `' +
        done +
        '`)'
    );
  }
  ...
  try {
    if (!_.isObject(options) || _.isArray(options) || _.isFunction(options)) {
      throw new Error(
        'Please provide a valid dictionary (plain JS object) as the 1st argument to `Waterline.start()`.  (Instead, got: `' +
          options +
          '`)'
      );
    }
    ...
    if (
      !_.isObject(options.adapters) ||
      _.isArray(options.adapters) ||
      _.isFunction(options.adapters)
    ) {
      throw new Error(
        '`adapters` must be provided as a valid dictionary (plain JS object) of adapter definitions, keyed by adapter identity.  (Instead, got: `' +
          options.adapters +
          '`)'
      );
    }
    if (
      !_.isObject(options.datastores) ||
      _.isArray(options.datastores) ||
      _.isFunction(options.datastores)
    ) {
      throw new Error(
        '`datastores` must be provided as a valid dictionary (plain JS object) of datastore configurations, keyed by datastore name.  (Instead, got: `' +
          options.datastores +
          '`)'
      );
    }
    if (
      !_.isObject(options.models) ||
      _.isArray(options.models) ||
      _.isFunction(options.models)
    ) {
      throw new Error(
        '`models` must be provided as a valid dictionary (plain JS object) of model definitions, keyed by model identity.  (Instead, got: `' +
          options.models +
          '`)'
      );
    }
    ...
    if (_.isUndefined(options.defaultModelSettings)) {
      ...
      !_.isObject(options.defaultModelSettings) ||
      _.isArray(options.defaultModelSettings) ||
      _.isFunction(options.defaultModelSettings)
    ) {
      throw new Error(
        'If specified, `defaultModelSettings` must be a dictionary (plain JavaScript object).  (Instead, got: `' +
          options.defaultModelSettings +
          '`)'
      );
    }
    ...
    _.each(options.adapters, function (adapter, key) {
      if (_.isUndefined(adapter.identity)) {
        ...
        throw new Error(
          'The `identity` explicitly defined on an adapter should exactly match the key under which it is passed in to `Waterline.start()`.  But the adapter passed in for key `' +
            key +
            '` has an identity that does not match: `' +
            adapter.identity +
            '`'
        );
      }
    ...
    _.each(options.models, function (userlandModelDef, key) {
      if (_.isUndefined(userlandModelDef.identity)) {
        ...
        throw new Error(
          'If `identity` is explicitly defined on a model definition, it should exactly match the key under which it is passed in to `Waterline.start()`.  But the model definition passed in for key `' +
            key +
            '` has an identity that does not match: `' +
            userlandModelDef.identity +
            '`'
        );
      }
      ...
      _.defaults(
        userlandModelDef,
        _.omit(options.defaultModelSettings, 'attributes')
      );
      if (options.defaultModelSettings.attributes) {
        userlandModelDef.attributes = userlandModelDef.attributes || {};
        _.defaults(
          userlandModelDef.attributes,
          options.defaultModelSettings.attributes
        );
      }
...
module.exports.stop = function (orm, done) {
  ...
  if (!_.isFunction(done)) {
    throw new Error(
      'Please provide a valid callback function as the 2nd argument to `Waterline.stop()`.  (Instead, got: `' +
        done +
        '`)'
    );
  }
  ...
  try {
    if (!_.isObject(orm)) {
      throw new Error(
        'Please provide a Waterline ORM instance (obtained from `Waterline.start()`) as the first argument to `Waterline.stop()`.  (Instead, got: `' +
          orm +
          '`)'
      );
    }
