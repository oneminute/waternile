...
module.exports = function CollectionBuilder(collection, datastores, context) {
  ...
  if (indexes.length > 0) {
    _.each(indexes, ind => {
      if (!_.has(ind, 'fields') || !_.isArray(ind.fields)) {
        throw new Error(
          `\n\n\incorrect index definition for model ${
            collection.prototype.tableName
          }. Each Item of array must contain the 'fields' Array property \n\n`
        );
      }
      ...
      _.each(ind.fields, (attribute, index) => {
        if (!_.includes(attributes, attribute)) {
          throw new Error(
            `\n\n\'${attribute}' is not a recognized attribute in model ${
              collection.prototype.tableName
            }. \n\n`
          );
        }
