  ...
  _.each(datastoreConfigs, function(config, datastoreName) {
    ...
    if (!_.has(config, 'adapter')) {
      throw new Error(
        'The datastore ' +
          datastoreName +
          ' is missing a required property (`adapter`). You should indicate the name of one of your adapters.'
      );
    }
    ...
    if (!_.has(adapters, config.adapter)) {
      ...
      if (!_.isString(config.adapter)) {
        throw new Error(
          'Invalid `adapter` property in datastore ' +
            datastoreName +
            '. It should be a string (the name of one of the adapters you passed into `waterline.initialize()`).'
        );
      }
      ...
      throw new Error(
        'Unknown adapter ' +
          ...
          ' for datastore ' +
          datastoreName +
          ". You should double-check that the connection's `adapter` property matches the name of one of your adapters. Or perhaps you forgot to include your adapter when you called `waterline.initialize()`.)"
      );
    }

    // Shallow-merge the adapter defaults underneath with the user-defined config.
    var datastoreConfig = _.extend(
      {},
      adapters[config.adapter].defaults,
      config
    );
