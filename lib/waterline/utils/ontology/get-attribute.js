...
module.exports = function getAttribute(attrName, modelIdentity, orm) {
  ...
  if (!_.isObject(attrDef) || _.isArray(attrDef) || _.isFunction(attrDef)) {
    throw new Error(
      'Consistency violation: The referenced attribute (`' +
        attrName +
        '`, from model `' +
        modelIdentity +
        '`) corresponds with a CORRUPTED attribute definition: ' +
        util.inspect(attrDef, { depth: 5 }) +
        ''
    );
  }
    ...
    if (!_.isString(attrDef.model) || attrDef.model === '') {
      throw new Error(
        'Consistency violation: The referenced attribute (`' +
          ...
          '`, from model `' +
          modelIdentity +
          '`) has an invalid `model` property.  If specified, `model` should be a non-empty string.  But instead, got: ' +
          util.inspect(attrDef.model, { depth: 5 }) +
          ''
      );
    }
    if (!_.isUndefined(attrDef.via)) {
      throw new Error(
        'Consistency violation: The referenced attribute (`' +
          ...
          '`, from model `' +
          modelIdentity +
          '`) is an association, because it declares a `model`.  But with a "model" association, the `via` property should always be undefined.  But instead, it is: ' +
          util.inspect(attrDef.via, { depth: 5 }) +
          ''
      );
    }
    if (!_.isUndefined(attrDef.dominant)) {
      throw new Error(
        'Consistency violation: The referenced attribute (`' +
          ...
          '`, from model `' +
          modelIdentity +
          '`) is an association, because it declares a `model`.  But with a "model" association, the `dominant` property should always be undefined.  But instead, it is: ' +
          util.inspect(attrDef.dominant, { depth: 5 }) +
          ''
      );
    }
    try {
      ...
      throw new Error(
        'Consistency violation: The referenced attribute (`' +
          ...
          '`, from model `' +
          modelIdentity +
          '`) is an association, because it declares a `model`.  But the other model it references (`' +
          attrDef.model +
          '`) is missing or invalid.  Details: ' +
          e.stack
      );
    }
  }
    ...
    if (!_.isString(attrDef.collection) || attrDef.collection === '') {
      throw new Error(
        'Consistency violation: The referenced attribute (`' +
          ...
          '`, from model `' +
          modelIdentity +
          '`) has an invalid `collection` property.  If specified, `collection` should be a non-empty string.  But instead, got: ' +
          util.inspect(attrDef.collection, { depth: 5 }) +
          ''
      );
    }
    ...
    try {
      ...
      throw new Error(
        'Consistency violation: The referenced attribute (`' +
          ...
          '`, from model `' +
          modelIdentity +
          '`) is an association, because it declares a `collection`.  But the other model it references (`' +
          attrDef.collection +
          '`) is missing or invalid.  Details: ' +
          e.stack
      );
      ...
      if (!_.isString(attrDef.via) || attrDef.via === '') {
        throw new Error(
          'Consistency violation: The referenced attribute (`' +
            ...
            '`, from model `' +
            modelIdentity +
            '`) has an invalid `via` property.  If specified, `via` should be a non-empty string.  But instead, got: ' +
            util.inspect(attrDef.via, { depth: 5 }) +
            ''
        );
      }
        ...
        try {
          ...
          throw new Error(
            'Consistency violation: The referenced attribute (`' +
              ...
              '`, from model `' +
              modelIdentity +
              '`) is a "through" association, because it declares a `through`.  But the junction model it references as "through" (`' +
              attrDef.through +
              '`) is missing or invalid.  Details: ' +
              e.stack
          );
        ...
        if (!ThroughWLModel.attributes[attrDef.via]) {
          throw new Error(
            'Consistency violation: The referenced attribute (`' +
              ...
              '`, from model `' +
              modelIdentity +
              '`) is a "through" association, because it declares a `through`.  But the association\'s specified `via` (' +
              attrDef.via +
              '`) does not correspond with a recognized attribute on the junction model (`' +
              attrDef.through +
              '`)'
          );
        }
        if (!ThroughWLModel.attributes[attrDef.via].model) {
          throw new Error(
            'Consistency violation: The referenced attribute (`' +
              ...
              '`, from model `' +
              modelIdentity +
              '`) is a "through" association, but its specified `via` (' +
              attrDef.via +
              '`) corresponds with an unexpected attribute on the junction model (`' +
              attrDef.through +
              '`).  The attribute referenced by `via` should be a singular ("model") association, but instead, got: ' +
              util.inspect(ThroughWLModel.attributes[attrDef.via], {
                depth: 5,
              }) +
              ''
          );
        ...
        if (!OtherWLModel.attributes[attrDef.via]) {
          throw new Error(
            'Consistency violation: The referenced attribute (`' +
              ...
              '`, from model `' +
              modelIdentity +
              '`) is an association, because it declares a `collection`.  But that association also specifies a `via` (' +
              attrDef.via +
              '`) which does not correspond with a recognized attribute on the other model (`' +
              attrDef.collection +
              '`)'
          );
        }
    ...
    if (!_.isString(attrDef.type) || attrDef.type === '') {
      throw new Error(
        'Consistency violation: The referenced attribute (`' +
          ...
          '`, from model `' +
          modelIdentity +
          '`) has an invalid `type` property.  If specified, `type` should be a non-empty string.  But instead, got: ' +
          util.inspect(attrDef.type, { depth: 5 }) +
          ''
      );
    }
    ...
    if (!_.isBoolean(attrDef.required)) {
      throw new Error(
        'Consistency violation: The referenced attribute (`' +
          ...
          '`, from model `' +
          modelIdentity +
          '`) has an unrecognized `required` property in its definition.  By this time, it should always be true or false.  But instead, got: ' +
          util.inspect(attrDef.required, { depth: 5 }) +
          ''
      );
    }
    if (attrDef.required && !_.isUndefined(attrDef.defaultsTo)) {
      throw new Error(
        'Consistency violation: The referenced attribute (`' +
          ...
          '`, from model `' +
          modelIdentity +
          '`) has `required: true`, but it also specifies a `defaultsTo`.  This should never have been allowed-- defaultsTo should be undefined!  But instead, got: ' +
          util.inspect(attrDef.defaultsTo, { depth: 5 }) +
          ''
      );
    }
  }
