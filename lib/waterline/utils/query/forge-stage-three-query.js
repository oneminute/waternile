...
module.exports = function forgeStageThreeQuery(options) {
  ...
  //   ╚╝ ╩ ╩╩═╝╩═╩╝╩ ╩ ╩ ╚═╝  └─┘┴   ┴ ┴└─┘┘└┘└─┘
  if (
    !_.has(options, 'stageTwoQuery') ||
    !_.isPlainObject(options.stageTwoQuery)
  ) {
    throw new Error(
      'Invalid options passed to `.buildStageThreeQuery()`. Missing or invalid `stageTwoQuery` option.'
    );
  }

  if (!_.has(options, 'identity') || !_.isString(options.identity)) {
    throw new Error(
      'Invalid options passed to `.buildStageThreeQuery()`. Missing or invalid `identity` option.'
    );
  }

  if (!_.has(options, 'transformer') || !_.isObject(options.transformer)) {
    throw new Error(
      'Invalid options passed to `.buildStageThreeQuery()`. Missing or invalid `transformer` option.'
    );
  }

  if (
    !_.has(options, 'originalModels') ||
    !_.isPlainObject(options.originalModels)
  ) {
    throw new Error(
      'Invalid options passed to `.buildStageThreeQuery()`. Missing or invalid `originalModels` option.'
    );
  }
  ...
  if (!model) {
    throw new Error(
      'A model with the identity ' +
        identity +
        ' could not be found in the schema. Perhaps the wrong schema was used?'
    );
  }
  ...
  if (s3Q.method === 'createEach' || s3Q.method === 'normalizeEach') {
    // Validate that there is a `newRecord` key on the object
  ...
  if (s3Q.method === 'update' || s3Q.method === 'upsert') {
    ...
    if (!_.has(s3Q, 'criteria') || !_.isPlainObject(s3Q.criteria)) {
      throw flaverr(
        'E_INVALID_RECORD',
        new Error('Failed process the criteria for the record.')
      );
    }
    ...
    if (!_.isUndefined(s3Q.criteria.sort) && s3Q.criteria.sort.length) {
      s3Q.criteria.sort = _.map(s3Q.criteria.sort, function (sortClause) {
        ...
        sort[
          [columnName].concat(comparatorTarget.split(/\./).slice(1)).join('.')
        ] = sortDirection;
        return sort;
      });
    }
  ...
  if (s3Q.method === 'find' || s3Q.method === 'findOne') {
    ...
    _.each(s3Q.populates, function (populateCriteria, populateAttribute) {
      ...
      if (
        _.isPlainObject(populateCriteria) &&
        !_.keys(populateCriteria).length
      ) {
        return;
      }
      ...
      try {
        ...
        if (parentAttr.collection && schemaAttribute.columnName) {
          console.warn(
            'Ignoring `columnName` setting for collection `' +
              attrDefToPopulate +
              '` on attribute `' +
              populateAttribute +
              '`.'
        ...
        var join = {
          ...
          parentKey:
            schemaAttribute.columnName ||
            model.schema[modelPrimaryKey].columnName,
        ...
        var select = [];
        var customSelect =
          populateCriteria.select && _.isArray(populateCriteria.select);

        // Expand out the `*` criteria
        if (
          customSelect &&
          populateCriteria.select.length === 1 &&
          _.first(populateCriteria.select) === '*'
        ) {
          customSelect = false;
        }

        _.each(originalModels[parentAttr.referenceIdentity].schema, function (
          val,
          key
        ) {
        ...
        if (referencedSchema.junctionTable) {
          join.select = false;
          reference = _.find(referencedSchema.schema, function (
            referencedPhysicalAttr
          ) {
            return (
              referencedPhysicalAttr.references &&
              referencedPhysicalAttr.columnName !== schemaAttribute.on
            );
          });
        }
        // If it's a through table, treat it the same way as a junction table for now
        else if (
          referencedSchema.throughTable &&
          referencedSchema.throughTable[identity + '.' + populateAttribute]
        ) {
          join.select = false;
          reference =
        ...
        // Otherwise apply any omits to the selected attributes
          ...
          if (
            ...
            _.isArray(populateCriteria.omit) &&
            populateCriteria.omit.length
          ) {
        ...
        if (reference && _.has(schemaAttribute, 'on')) {
          var selects = [];
          _.each(originalModels[reference.referenceIdentity].schema, function (
            val,
            key
          ) {
            // Ignore virtual attributes
    ...
    // remove the select.
    if (
      (model.hasSchema === false && _.indexOf(s3Q.criteria.select, '*') > -1) ||
      (s3Q.meta && s3Q.meta.skipExpandingDefaultSelectClause)
    ) {
      delete s3Q.criteria.select;
    }
    ...
    _.each(s3Q.joins, function (join) {
      ...
      join.criteria = joinCollection._transformer.serializeCriteria(
        join.criteria
      );
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // Custom methods for ArangoJs added here
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  if (
    ...
    s3Q.method === 'count' ||
    s3Q.method === 'findAny' ||
    s3Q.method === 'findInbound' ||
    s3Q.method === 'findOutbound'
  ) {
    // Validate that there is a `criteria` key on the object
    if (!_.has(s3Q, 'criteria') || !_.isPlainObject(s3Q.criteria)) {
      throw flaverr(
    ...
    try {
      if (_.isString(s3Q.numericAttrName)) {
  ...
  // findNear
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  if (s3Q.method === 'findNear' || s3Q.method === 'sample') {
    // Validate that there is a `criteria` key on the object
    if (!_.has(s3Q, 'criteria') || !_.isPlainObject(s3Q.criteria)) {
      throw flaverr(
        'E_INVALID_RECORD',
  ...
  // aggregate
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  if (s3Q.method === 'aggregate') {
    // Validate that there is a `criteria` key on the object
    if (
      !_.has(s3Q, 'aggregateCriteria') ||
      (!_.isPlainObject(s3Q.aggregateCriteria) &&
        !_.isArray(s3Q.aggregateCriteria))
    ) {
      throw flaverr(
        'E_INVALID_RECORD',
        new Error(
          '\n\nFailed process the aggregateCriteria for the record.\n\n'
        )
      );
    }
  ...
  throw flaverr(
    'E_INVALID_QUERY',
    new Error('Invalid query method set - `' + s3Q.method + '`.')
  );
};
