  ...
  if (
    ...
    _.each(s3Q.populates, function (populateCriteria, populateAttribute) {
      ...
      try {
        ...
        _.each(
          originalModels[parentAttr.referenceIdentity].schema,
          function (val, key) {
        ...
        if (referencedSchema.junctionTable) {
          ...
          reference = _.find(
            ...
            function (referencedPhysicalAttr) {
              return (
        ...
        if (reference && _.has(schemaAttribute, 'on')) {
          var selects = [];
          _.each(
            originalModels[reference.referenceIdentity].schema,
            function (val, key) {
              // Ignore virtual attributes
              if (_.has(val, 'collection')) {
                return;
