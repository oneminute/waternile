...
module.exports = function helpFind(WLModel, s2q, omen, done) {
  if (!_.isFunction(done)) {
    throw new Error(
      'Consistency violation: `done` (4th argument) should be a function'
    );
  }
  if (!WLModel) {
    return done(
      new Error(
        'Consistency violation: Live Waterline model should be provided as the 1st argument'
      )
    );
  }
  if (!s2q) {
    return done(
      new Error(
        'Consistency violation: Stage two query (S2Q) should be provided as the 2nd argument'
      )
    );
  }
  if (!omen) {
    return done(
      new Error(
        'Consistency violation: Omen should be provided as the 3rd argument'
      )
    );
  }
  ...
  (function _getPopulatedRecords(proceed) {
    ...
    var doJoinsInParentAdapter = (function () {
      ...
      return (
        ...
        _.all(parentQuery.joins, function (join) {
          ...
          return (
            collections[join.childCollectionIdentity].datastore ===
            WLModel.datastore
          );
        })
      );
    ...
    if (doJoinsInParentAdapter) {
      ...
      parentAdapter.join(parentDatastoreName, parentQuery, function (
        err,
        rawResultFromAdapter
      ) {
        if (err) {
    ...
    // results and proceed.
      ...
      parentAdapter.find(parentDatastoreName, parentQuery, function (
        err,
        rawResultFromAdapter
      ) {
        if (err) {
    ...
    // Otherwise we have some joining to do...
      ...
      var joinsByAlias = _.groupBy(parentQuery.joins, function (join) {
        return join.alias;
      });
      ...
      parentAdapter.find(
        ...
        function (err, parentResults) {
          ...
          async.reduce(
            _.keys(joinsByAlias),
            parentResults,
            function (populatedParentRecords, alias, nextSetOfJoins) {
              ...
              if (aliasJoins.length === 2) {
                ...
                var firstJoin = _.first(
                  _.remove(aliasJoins, function (join) {
                    return join.parentCollectionIdentity === WLModel.identity;
                  })
                );
                ...
                var junctionTableModel =
                  collections[firstJoin.childCollectionIdentity];
                var junctionTablePrimaryKeyName = junctionTableModel.primaryKey;
                var junctionTablePrimaryKeyColumnName =
                  junctionTableModel.schema[junctionTablePrimaryKeyName]
                    .columnName;
                ...
                var junctionTableQuery = {
                  ...
                  criteria: {
                    ...
                    select: [
                      junctionTablePrimaryKeyColumnName,
                      firstJoin.childKey,
                      secondJoin.parentKey,
                    ],
                  },
                ...
                junctionTableQueryInConjunct[firstJoin.childKey] = {
                  in: _.pluck(parentResults, firstJoin.parentKey),
                };
                junctionTableQuery.criteria.where.and.push(
                  junctionTableQueryInConjunct
                );
                ...
                junctionTableAdapter.find(
                  junctionTableDatastoreName,
                  junctionTableQuery,
                  function (err, junctionTableResults) {
                    if (err) {
                    ...
                    var childTableModel =
                      collections[secondJoin.childCollectionIdentity];
                    ...
                    if (_.keys(secondJoin.criteria.where).length > 0) {
                      ...
                      if (secondJoin.criteria.where.and) {
                        baseChildTableQuery.criteria.where.and = baseChildTableQuery.criteria.where.and.concat(
                          secondJoin.criteria.where.and
                        );
                      } else {
                        ...
                        baseChildTableQuery.criteria.where.and.push(
                          secondJoin.criteria.where
                        );
                      }
                    }
                    ...
                    if (!_.isUndefined(secondJoin.criteria.skip)) {
                      baseChildTableQuery.criteria.skip =
                        secondJoin.criteria.skip;
                    } else {
                    ...
                    if (!_.isUndefined(secondJoin.criteria.limit)) {
                      baseChildTableQuery.criteria.limit =
                        secondJoin.criteria.limit;
                    } else {
                      baseChildTableQuery.criteria.limit =
                    ...
                    if (!_.isUndefined(secondJoin.criteria.select)) {
                      baseChildTableQuery.criteria.select =
                        secondJoin.criteria.select;
                    }

                    // Get the unique parent primary keys from the junction table result.
                    var parentPks = _.uniq(
                      _.pluck(junctionTableResults, firstJoin.childKey)
                    );
                    ...
                    async.reduce(
                      parentPks,
                      {},
                      function (memo, parentPk, nextParentPk) {
                        ...
                        var junctionTableRecordsForThisParent = _.filter(
                        ...
                        var childPks = _.pluck(
                          junctionTableRecordsForThisParent,
                          secondJoin.parentKey
                        );
                        ...
                        childTableQuery.criteria.where.and.push(
                          childInConjunct
                        );
                        ...
                        childTableAdapter.find(
                          childTableDatastoreName,
                          childTableQuery,
                          function (err, childTableResults) {
                            if (err) {
                              // Note that we're careful to use the identity, not the table name!
                              err = forgeAdapterError(
                                err,
                      ...
                      function _afterGettingChildRecords(
                        err,
                        ...
                        return nextSetOfJoins(
                          undefined,
                          populatedParentRecords
                        );
                      }
                    ); // </ async.reduce() >
                  }
              ...
              // Otherwise, if there's one join in the set: no junction table.
                ...
                // Get a reference to the child table model.
                var childTableModel =
                  collections[singleJoin.childCollectionIdentity];
                ...
                async.map(
                  populatedParentRecords,
                  function (parentRecord, nextParentRecord) {
                    ...
                    var pkConjunct = {};
                    pkConjunct[singleJoin.childKey] =
                      parentRecord[singleJoin.parentKey];
              ...
              // Otherwise, if we don't have either 1 or 2 joins for the alias.  That's a prOblEm!!?!
                ...
                return nextSetOfJoins(
                  new Error(
                    'Consistency violation: the alias `' +
                      alias +
                      '` should have either 1 or 2 joins, but instead had ' +
                      aliasJoins.length +
                      '!'
                  )
                );
    ...
    populatedRecords = _.map(populatedRecords, function (
      populatedPhysicalRecord
    ) {
    ...
    if (!_.isArray(joins)) {
      return done(
        new Error(
          'Consistency violation: `joins` must be an array at this point.  But instead, somehow it is this: ' +
            util.inspect(joins, {
              depth: 5,
            }) +
            ''
        ...
        _.each(_.keys(record), function (key) {
          ...
          var joinModelIdentity = (function () {
            ...
            if (joinsUsingThisAlias.length === 2) {
              return _.find(joins, {
                parentCollectionIdentity: referenceIdentity,
          ...
          if (!_.isArray(record[key])) {
            if (!_.isNull(record[key]) && !_.isObject(record[key])) {
              throw new Error(
                "Consistency violation: IWMIH, `record['" +
                  "']` should always be either `null` (if populating failed) or a dictionary (if it worked).  But instead, got: " +
                  util.inspect(record[key], { depth: 5 }) +
                  ''
              );
            }
          ...
          _.each(record[key], function (originalChildRecord) {
            ...
            transformedChildRecord = WLChildModel._transformer.unserialize(
              originalChildRecord
            );
    ...
    if (!_.isArray(populatedRecords)) {
      return done(
        new Error(
          'Consistency violation: Result from helpFind() utility should be an array, but instead got: ' +
            util.inspect(populatedRecords, {
              depth: 5,
            }) +
            ''
        )
