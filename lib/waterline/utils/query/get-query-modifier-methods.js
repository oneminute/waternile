...
var BASELINE_Q_METHODS = {
  ...
  //   if (!this._alreadyDeclaredVariables) {
  //     this._wlQueryInfo.criteria = expandWhereShorthand(
  //       this._wlQueryInfo.criteria
  //     );
  //     this._alreadyDeclaredVariables = true;
  //   } //>-

  //   this._wlQueryInfo.criteria.let = variables;
  //   return this;
  // },
...
var STREAM_Q_METHODS = {
  ...
  eachRecord: function (iteratee) {
    assert(
      this._wlQueryInfo.method === 'stream',
      'Cannot chain `.eachRecord()` onto the `.' +
        this._wlQueryInfo.method +
        "()` method.  The `.eachRecord()` method is only chainable to `.stream()`.  (In fact, this shouldn't even be possible!  So the fact that you are seeing this message at all is, itself, likely due to a bug in Waterline.)"
    );
  ...
  eachBatch: function (batchSizeOrIteratee, iteratee) {
    assert(
      this._wlQueryInfo.method === 'stream',
      'Cannot chain `.eachBatch()` onto the `.' +
        this._wlQueryInfo.method +
        "()` method.  The `.eachBatch()` method is only chainable to `.stream()`.  (In fact, this shouldn't even be possible!  So the fact that you are seeing this message at all is, itself, likely due to a bug in Waterline.)"
    );

    if (arguments.length > 2) {
      throw new Error(
        'Invalid usage for `.eachBatch()` -- no more than 2 arguments should be passed in.'
      );
    } //•
...
var POPULATE_Q_METHODS = {
  ...
  populate: function (keyName, subcriteria) {
    assert(
      this._wlQueryInfo.method === 'find' ||
        this._wlQueryInfo.method === 'findOne' ||
        this._wlQueryInfo.method === 'findWithCount' ||
        this._wlQueryInfo.method === 'stream',
      'Cannot chain `.populate()` onto the `.' +
        this._wlQueryInfo.method +
        "()` method.  (In fact, this shouldn't even be possible!  So the fact that you are seeing this message at all is, itself, likely due to a bug in Waterline.)"
    );
    ...
    if (!keyName || !_.isString(keyName)) {
      throw new Error(
        'Invalid usage for `.populate()` -- first argument should be the name of an assocation.'
      );
    }
...
var PAGINATION_Q_METHODS = {
  ...
  paginate: function (pageNumOrOpts, pageSize) {
    ...
    // If dictionary... (temporary backwards-compat.)
      ...
      console.warn(
        'Deprecation warning: Passing in a dictionary (plain JS object) to .paginate()\n' +
          ...
          '```\n' +
          '(In the mean time, interpreting this as page #' +
          pageNum +
          '...)'
      );
    }
    ...
    if (!_.isUndefined(pageSize)) {
      ...
      _.isObject(pageNumOrOpts) &&
      !_.isUndefined(pageNumOrOpts.limit)
    ) {
    ...
    this.skip(pageNum * pageSize).limit(pageSize);
...
var FILTER_VERTEX_METHODS = {
  ...
  whereVertex: function (whereCriteria) {
    if (!this._alreadyInitiallyExpandedCriteria) {
      this._wlQueryInfo.criteria = expandWhereShorthand(
        this._wlQueryInfo.criteria
      );
    ...
    this._wlQueryInfo.criteria.whereVertex = whereCriteria;

    return this;
  },
};

var FILTER_EDGE_METHODS = {
  /**
   * Add a `where` clause to the query's criteria.
   *
   * @param {Dictionary} criteria to append
  ...
  whereEdge: function (whereCriteria) {
    if (!this._alreadyInitiallyExpandedCriteria) {
      this._wlQueryInfo.criteria = expandWhereShorthand(
        this._wlQueryInfo.criteria
      );
    ...
    this._wlQueryInfo.criteria.whereEdge = whereCriteria;
...
var FETCH_Q_METHODS = {
  ...
  fetch: function () {
    if (arguments.length > 0) {
      throw new Error(
        'Invalid usage for `.fetch()` -- no arguments should be passed in.'
      );
    }
...
var DECRYPT_Q_METHODS = {
  ...
  decrypt: function () {
    if (arguments.length > 0) {
      throw new Error(
        'Invalid usage for `.decrypt()` -- no arguments should be passed in.'
      );
    }
...
module.exports = function getQueryModifierMethods(category) {
  assert(
    category && _.isString(category),
    'A category must be provided as a valid string.'
  );
  ...
  switch (category) {
    case 'find':
      _.extend(
        queryMethods,
        FILTER_Q_METHODS,
        PAGINATION_Q_METHODS,
        OLD_AGGREGATION_Q_METHODS,
        PROJECTION_Q_METHODS,
        POPULATE_Q_METHODS,
        DECRYPT_Q_METHODS
      );
      break;
    ...
    case 'stream':
      ...
      break;
    case 'count':
      _.extend(queryMethods, FILTER_Q_METHODS);
      break;
    ...
    //Edited to support graphs by gaithoben.
    case 'createEdge':
      _.extend(queryMethods, SET_Q_METHODS, FETCH_Q_METHODS, DECRYPT_Q_METHODS);
      break;
    case 'findOutbound':
      _.extend(
        queryMethods,
        FILTER_Q_METHODS,
        FILTER_VERTEX_METHODS,
        FILTER_EDGE_METHODS,
        PAGINATION_Q_METHODS
      );
      break;
    ...
    case 'findNear':
      ...
      break;
    case 'aggregate':
      _.extend(queryMethods, DECRYPT_Q_METHODS);
      break;
    ...
    case 'addToCollection':
      _.extend(queryMethods, COLLECTION_Q_METHODS);
      break;
    ...
    default:
      throw new Error(
        'Consistency violation: Unrecognized category (model method name): `' +
          category +
          '`'
      );
  }
