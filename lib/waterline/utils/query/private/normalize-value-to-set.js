 ...
 */
module.exports = function normalizeValueToSet(
  value,
  supposedAttrName,
  modelIdentity,
  orm,
  ...
  try {
    ...
    switch (e.code) {
      case 'E_MODEL_NOT_REGISTERED':
        throw new Error('Consistency violation: ' + e.message);
      default:
        throw e;
    }
  } //</catch>
  ...
  // Else if this model declares `schema: false`...
    ...
    if (!isValidAttributeName(supposedAttrName)) {
      if (supposedAttrName === '') {
        throw flaverr(
          'E_HIGHLY_IRREGULAR',
          new Error("Empty string ('') is not a valid name for an attribute.")
        );
      } else {
        throw flaverr(
          'E_HIGHLY_IRREGULAR',
          new Error('This is not a valid name for an attribute.')
        );
      }
    } //-•
  }
  // ‡
    ...
    throw new Error(
      'Consistency violation: Every live Waterline model should always have the `hasSchema` flag ' +
        ...
        "But somehow, this model's (`" +
        modelIdentity +
        '`) `hasSchema` property is as follows: ' +
  ...
  if (!correspondingAttrDef) {
    ...
    try {
      if (`${value}`.includes('record.') || `${value}`.slice(0, 1) === '$') {
        // Nothing to validate
      } else {
      ...
      switch (e.code) {
        case 'E_INVALID':
          throw flaverr(
            { code: 'E_TYPE', expectedType: 'json' },
            new Error(
              'Invalid value for unrecognized attribute (must be JSON-compatible).  To explicitly allow ' +
                'non-JSON-compatible values like this, define a `' +
                supposedAttrName +
                '` attribute, and specify ' +
                '`type: ref`.  More info on this error: ' +
                e.message
            )
          );
        default:
    ...
    try {
      ...
      switch (e.code) {
        case 'E_INVALID_PK_VALUE':
          throw flaverr(
            'E_HIGHLY_IRREGULAR',
            new Error('Invalid primary key value.  ' + e.message)
          );
  ...
  //  └  └─┘┴└─  ╩  ╩═╝╚═╝╩╚═╩ ╩╩═╝  ╩ ╩╚═╝╚═╝╚═╝╚═╝╩╩ ╩ ╩ ╩╚═╝╝╚╝
    ...
    try {
      value = normalizePkValueOrValues(
        value,
        getAttribute(
          getModel(correspondingAttrDef.collection, orm).primaryKey,
          correspondingAttrDef.collection,
          orm
        ).type
      );
    } catch (e) {
  ...
  //  └  └─┘┴└─  ╚═╝╩╝╚╝╚═╝╚═╝╩═╝╩ ╩╩╚═  ╩ ╩╚═╝╚═╝╚═╝╚═╝╩╩ ╩ ╩ ╩╚═╝╝╚╝
    ...
    if (_.isNull(value)) {
      ...
      if (correspondingAttrDef.required) {
        throw flaverr(
          'E_REQUIRED',
          new Error(
            'Cannot set `null` for required association (`' +
              supposedAttrName +
              '`).'
          )
        );
    ...
    // of the associated model (normalizing it, if appropriate/possible.)
      ...
      try {
        value = normalizePkValue(
          value,
          getAttribute(
            getModel(correspondingAttrDef.model, orm).primaryKey,
            correspondingAttrDef.model,
            orm
          ).type
        );
      } catch (e) {
  ...
  // > We'll use loose validation (& thus also light coercion) on the value and see what happens.
    ...
    if (
      !_.isString(correspondingAttrDef.type) ||
      correspondingAttrDef.type === ''
    ) {
      throw new Error(
        'Consistency violation: There is no way this attribute (`' +
          supposedAttrName +
          '`) should have been allowed to be registered with neither a `type`, `model`, nor `collection`!  Here is the attr def: ' +
          util.inspect(correspondingAttrDef, { depth: 5 }) +
          ''
      );
    }
    ...
    if (
      correspondingAttrDef.autoCreatedAt ||
      correspondingAttrDef.autoUpdatedAt
    ) {
    ...
    if (isProvidingNullForIncompatibleOptionalAttr) {
      throw flaverr(
        { code: 'E_TYPE', expectedType: correspondingAttrDef.type },
        new Error(
          'Specified value (`null`) is not a valid `' +
            supposedAttrName +
            '`.  ' +
            ...
            (function _getBaseValuePhrase() {
              switch (correspondingAttrDef.type) {
                ...
                case 'number':
                  return '`0` (zero)';
                default:
                  return '`' + rttc.coerce(correspondingAttrDef.type) + '`';
              }
            })() +
            ...
            (function _getExtraPhrase() {
              if (_.isUndefined(correspondingAttrDef.defaultsTo)) {
                return (
                  'Also note: Since this attribute does not define a `defaultsTo`, ' +
                  'the base value will be used as an implicit default if `' +
                  supposedAttrName +
                  '` ' +
        ...
        switch (e.code) {
          case 'E_INVALID':
            throw flaverr(
              { code: 'E_TYPE', expectedType: correspondingAttrDef.type },
              new Error(
                'Da Specified value is not a valid `' +
                  supposedAttrName +
                  '`.  ' +
                  e.message
              )
    ...
    if (correspondingAttrDef.required) {
      ...
      if (value === '') {
        throw flaverr(
          'E_REQUIRED',
          new Error('Cannot set "" (empty string) for a required attribute.')
        );
      } //>-•
      ...
      if (_.isNull(value)) {
        throw flaverr(
          'E_REQUIRED',
          new Error('Cannot set `null` for a required attribute.')
        );
      } //-•
    } //>-   </ if required >
    ...
    doCheckForRuleViolations =
      !_.isNull(value) && !_.isUndefined(correspondingAttrDef.validations);
  ...
  if (doCheckForRuleViolations) {
    ...
    var isRulesetDictionary =
      _.isObject(ruleset) && !_.isArray(ruleset) && !_.isFunction(ruleset);
    if (!isRulesetDictionary) {
      throw new Error(
        "Consistency violation: If set, an attribute's validations ruleset (`validations`) should always be a dictionary (plain JavaScript object).  But for the `" +
          modelIdentity +
          "` model's `" +
          supposedAttrName +
          '` attribute, it somehow ended up as this instead: ' +
          util.inspect(correspondingAttrDef.validations, { depth: 5 }) +
          ''
      );
    }
    ...
    try {
      ...
      throw new Error(
        'Consistency violation: Unexpected error occurred when attempting to apply ' +
          'high-level validation rules from `' +
          modelIdentity +
          "` model's `" +
          supposedAttrName +
          '` ' +
    ...
    if (ruleViolations.length > 0) {
      ...
      throw flaverr(
        ...
        new Error('Violated one or more validation rules:\n' + summary)
      );
    } //-•
  } //>-•  </if (doCheckForRuleViolations) >
  ...
  if (correspondingAttrDef && correspondingAttrDef.encrypt) {
    ...
    if (
      !_.isObject(WLModel.dataEncryptionKeys) ||
      !WLModel.dataEncryptionKeys.default ||
      !_.isString(WLModel.dataEncryptionKeys.default)
    ) {
      throw new Error(
        'Consistency violation: `' +
          modelIdentity +
          '` model has a corrupted definition.  Should not ' +
    ...
    if (!WLModel.dataEncryptionKeys[idOfDekToEncryptWith]) {
      throw new Error(
        'There is no known data encryption key by that name (`' +
          idOfDekToEncryptWith +
          '`).  ' +
      ...
      // > see https://github.com/balderdashy/sails/issues/4302#issuecomment-363883885)
        ...
        try {
          ...
          throw flaverr(
            {
              message:
                'Before encrypting, Waterline attempted to JSON-stringify this value to ensure it ' +
                "could be accurately decoded into the correct data type later (for example, `2` vs `'2'`).  " +
                'But this time, JSON.stringify() failed with the following error:  ' +
                err.message,
            },
            err
          );
