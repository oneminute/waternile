...
module.exports = function normalizeSortClause(
  sortClause,
  modelIdentity,
  orm,
  meta
  ...
  if (
    _.isObject(sortClause) &&
    !_.isArray(sortClause) &&
    !_.isFunction(sortClause)
  ) {
    sortClause = _.reduce(
      _.keys(sortClause),
      function(memo, sortByKey) {
  ...
  sortClause = _.map(sortClause, function(comparatorDirective) {
    ...
    if (
      !_.isObject(comparatorDirective) ||
      _.isArray(comparatorDirective) ||
      _.isFunction(comparatorDirective)
    ) {
      throw flaverr(
        'E_SORT_CLAUSE_UNUSABLE',
        new Error(
        ...
        throw flaverr(
          ...
          new Error(
            'The `sort` clause in the provided criteria is invalid, because, although it ' +
              ...
              _.keys(comparatorDirective).length +
              ' keys...  ' +
        ...
        throw flaverr(
          ...
          new Error(
            'The `sort` clause in the provided criteria is invalid, because, although it ' +
              ...
              'It indicates that we should sort by `' +
              attrName +
              '`-- but that is not a recognized ' +
              'attribute for this model (`' +
              modelIdentity +
              '`).  Since the model declares `schema: true`, ' +
              'this is not allowed.'
          )
        );
      } //-•
    ...
    if (attrDef && attrDef.collection) {
      throw flaverr(
        ...
        new Error(
          'Cannot sort by `' +
            attrName +
            '` because it corresponds with an "unsortable" attribute ' +
            'definition for this model (`' +
            modelIdentity +
            '`).  This attribute is a plural (`collection`) ' +
    ...
    if (isDeepTarget) {
      ...
      if (
        ...
        throw flaverr(
          'E_SORT_CLAUSE_UNUSABLE',
          new Error(
            'Cannot use dot notation to sort by a nested property of `' +
              attrName +
              '` because ' +
    ...
    if (_.isString(comparatorDirective[sortByKey])) {
      comparatorDirective[sortByKey] = comparatorDirective[
        sortByKey
      ].toUpperCase();
    } //>-
        ...
        throw flaverr(
          ...
          new Error(
            'The `sort` clause in the provided criteria is invalid, because, although it ' +
              ...
              'it suggests that Waterline should use `' +
              comparatorDirective[sortByKey] +
              '` ' +
  ...
  _.each(sortClause, function(comparatorDirective) {
    ...
    if (_.contains(referencedComparatorTargets, sortByKey)) {
      throw flaverr(
        'E_SORT_CLAUSE_UNUSABLE',
        new Error('Cannot sort by the same thing (`' + sortByKey + '`) twice!')
      );
    } //-•
