...
var NAMES_OF_RECOGNIZED_CLAUSES = [
  'let',
  'where',
  'whereVertex',
  'whereEdge',
 ...
 */
module.exports = function normalizeCriteria(
  criteria,
  modelIdentity,
  orm,
  meta
  ...
  if (_.isUndefined(criteria)) {
    throw new Error(
      'Consistency violation: `criteria` should never be `undefined` when it is passed in to the normalizeCriteria() utility.'
    );
  }
  ...
  if (!_.isObject(criteria) || _.isArray(criteria) || _.isFunction(criteria)) {
    throw flaverr(
      'E_HIGHLY_IRREGULAR',
      new Error(
        'The provided criteria is invalid.  Should be a dictionary (plain JavaScript object), but instead got: ' +
          util.inspect(criteria, { depth: 5 }) +
          ''
      )
    );
  ...
  var recognizedClauses = _.intersection(
    _.keys(criteria),
  ...
  try {
    criteria.where = normalizeWhereClause(
      criteria.where,
      modelIdentity,
      orm,
      meta
    ...
    switch (e.code) {
      case 'E_WHERE_CLAUSE_UNUSABLE':
        throw flaverr(
          'E_HIGHLY_IRREGULAR',
          new Error('Could not use the provided `where` clause.  ' + e.message)
        );
      ...
      default:
        throw new Error(
          'Consistency violation: Unexpected error normalizing/validating the `where` clause: ' +
            e.stack
        );
    }
  } //>-•
  ...
  if (criteria.limit === 0) {
    throw flaverr(
      'E_WOULD_RESULT_IN_NOTHING',
      new Error(
        'A criteria with `limit: 0` will never actually match any records.'
      )
    );
  } //-•
  ...
  if (criteria.skip === 0) {
    /* skip: 0 is valid */
  } else if (isSafeNaturalNumber(criteria.skip)) {
    /* any safe, natural number is a valid `skip` */
  } else {
    throw flaverr(
      'E_HIGHLY_IRREGULAR',
      new Error(
  ...
  try {
    criteria.sort = normalizeSortClause(
      criteria.sort,
      modelIdentity,
      orm,
      meta
    ...
    switch (e.code) {
      case 'E_SORT_CLAUSE_UNUSABLE':
        throw flaverr(
          'E_HIGHLY_IRREGULAR',
          new Error('Could not use the provided `sort` clause: ' + e.message)
        );
      ...
      default:
        throw new Error(
          'Consistency violation: Encountered unexpected internal error when attempting to normalize/validate a provided `sort` clause:\n```\n' +
  ...
  // Otherwise, we must investigate further.
    ...
    //EDITED BY GAITHO. This was commented to allow select clause with schemaless models.
      ...
      if (WLModel.hasSchema === true) {
        ...
        if (!attrDef) {
          throw flaverr(
            'E_HIGHLY_IRREGULAR',
            new Error(
              'The `select` clause in the provided criteria contains an item (`' +
                attrNameToKeep +
                '`) which is ' +
                'not a recognized attribute in this model (`' +
                modelIdentity +
                '`).'
            )
          );
        ...
        throw flaverr(
          ...
          new Error(
            'The `select` clause in the provided criteria contains an item (`' +
              attrNameToKeep +
              '`) which is actually ' +
              'the name of a plural ("collection") association for this model (`' +
              modelIdentity +
              '`).  But you cannot ' +
  ...
  _.remove(criteria.omit, function (attrNameToOmit) {
    ...
    if (attrNameToOmit === WLModel.primaryKey) {
      throw flaverr(
        'E_HIGHLY_IRREGULAR',
        new Error(
          'The `omit` clause in the provided criteria explicitly attempts to omit the primary key (`' +
            WLModel.primaryKey +
            '`).  But in the current version of Waterline, this is not possible.'
        )
      );
    } //-•
    ...
    if (WLModel.hasSchema === true) {
      ...
      if (!attrDef) {
        throw flaverr(
          'E_HIGHLY_IRREGULAR',
          new Error(
            'The `omit` clause in the provided criteria contains an item (`' +
              attrNameToOmit +
              '`) which is not a recognized attribute in this model (`' +
              modelIdentity +
              '`).'
          )
        );
    ...
    // Else if model is `schema: false`...
      ...
      throw flaverr(
        'E_HIGHLY_IRREGULAR',
        new Error(
          'Cannot use `omit`, because the referenced model (`' +
            modelIdentity +
            '`) does not declare itself `schema: true`.'
        )
      );
  ...
  if (!isNoopSelect && !isNoopOmit) {
    throw flaverr(
      'E_HIGHLY_IRREGULAR',
      new Error(
        'Cannot specify both `omit` AND `select`.  Please use one or the other.'
      )
    );
  } //-•
  ...
  var isCriteriaNowValidDictionary =
    _.isObject(criteria) && !_.isArray(criteria) && !_.isFunction(criteria);
  if (!isCriteriaNowValidDictionary) {
    throw new Error(
      'Consistency violation: At this point, the criteria should have already been normalized into a dictionary!  But instead somehow it looks like this: ' +
        util.inspect(criteria, { depth: 5 }) +
        ''
    );
  }
