...
module.exports = function normalizeComparisonValue(
  value,
  attrName,
  modelIdentity,
  orm
  ...
  if (!_.isString(attrName)) {
    throw new Error(
      'Consistency violation: This internal utility must always be called with a valid second argument (the attribute name).  But instead, got: ' +
        util.inspect(attrName, { depth: 5 }) +
        ''
    );
  }
  if (!_.isString(modelIdentity)) {
    throw new Error(
      'Consistency violation: This internal utility must always be called with a valid third argument (the model identity).  But instead, got: ' +
  ...
  if (!_.isObject(orm)) {
    throw new Error(
      'Consistency violation: This internal utility must always be called with a valid fourth argument (the orm instance).  But instead, got: ' +
        util.inspect(orm, { depth: 5 }) +
        ''
    );
  }
  ...
  if (attrDef) {
    assert(
      !attrDef.collection,
      'Should not call this internal utility on a plural association (i.e. `collection` attribute).'
    );
  }
          ...
          throw flaverr(
            ...
            new Error(
              'There is no such attribute declared by this model... which is fine, ' +
                ...
                'must be JSON-compatible, and this one is not.  ' +
                e.message
            )
          );
  ...
  //  └  └─┘┴└─  ╚═╝╩╝╚╝╚═╝╚═╝╩═╝╩ ╩╩╚═  ╩ ╩╚═╝╚═╝╚═╝╚═╝╩╩ ╩ ╩ ╩╚═╝╝╚╝
    ...
    var associatedPkType = getAttribute(
      getModel(attrDef.model, orm).primaryKey,
      attrDef.model,
      orm
    ).type;
    try {
          ...
          throw flaverr(
            ...
            new Error(
              'The corresponding attribute (`' +
                attrName +
                '`) is a singular ("model") association, ' +
      ...
      switch (e.code) {
        case 'E_INVALID':
          throw flaverr(
            'E_VALUE_NOT_USABLE',
            new Error(
              'Does not match the declared data type of the corresponding attribute.  ' +
                e.message
            )
          );
