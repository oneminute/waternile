...
module.exports = function normalizeConstraint(
  constraintRhs,
  constraintTarget,
  modelIdentity,
  orm,
  ...
  if (_.isUndefined(constraintRhs)) {
    throw new Error(
      'Consistency violation: The internal normalizeConstraint() utility must always be called with a first argument (the RHS of the constraint to normalize).  But instead, got: ' +
        util.inspect(constraintRhs, { depth: 5 }) +
        ''
    );
  }
  if (!_.isString(constraintTarget)) {
    throw new Error(
      'Consistency violation: The internal normalizeConstraint() utility must always be called with a valid second argument (a string).  But instead, got: ' +
        util.inspect(constraintTarget, { depth: 5 }) +
        ''
    );
  }
  if (!_.isString(modelIdentity)) {
    throw new Error(
      'Consistency violation: The internal normalizeConstraint() utility must always be called with a valid third argument (a string).  But instead, got: ' +
        util.inspect(modelIdentity, { depth: 5 }) +
        ''
    );
  }
  ...
  //EDITED by GAITHO
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // ENABBLE enableExperimentalDeepTargets by default
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  meta = { enableExperimentalDeepTargets: true, ...meta };
    ...
    if (!attrDef) {
      throw flaverr(
        ...
        new Error(
          '`' +
            attrName +
            '` is not a recognized attribute for this ' +
            'model (`' +
            modelIdentity +
            '`).  And since the model declares `schema: true`, ' +
            'this is not allowed.'
        )
      );
    } //-•
  ...
  // Else if model is `schema: false`...
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // Edited bbyy Gaitho. To allow querying of Aray values in ArangoDB
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    const stripped = attrName.replace('[*]', '');

    if (!isValidAttributeName(stripped)) {
      throw flaverr(
        'E_CONSTRAINT_NOT_USABLE',
        new Error(
          '`' +
            attrName +
            '` is not a valid name for an attribute in Waterline.  ' +
            'Even though this model (`' +
            modelIdentity +
            '`) declares `schema: false`, ' +
            'this is not allowed.'
        )
      );
    } //-•
    ...
    throw new Error(
      'Consistency violation: Every instantiated Waterline model should always have a `hasSchema` property as either `true` or `false` (should have been derived from the `schema` model setting when Waterline was being initialized).  But somehow, this model (`' +
        modelIdentity +
        '`) ended up with `hasSchema: ' +
  ...
  if (attrDef && attrDef.collection) {
    throw flaverr(
      ...
      new Error(
        'Cannot filter by `' +
          attrName +
          "` because it is a plural association (which wouldn't make sense)."
      )
    );
  } //-•
  ...
  if (isDeepTarget) {
    ...
    if (
      ...
      throw flaverr(
        'E_CONSTRAINT_NOT_USABLE',
        new Error(
          'Cannot use dot notation in a constraint for the `' +
            attrName +
            '` attribute.  ' +
  ...
  if (
    _.isObject(constraintRhs) &&
    !_.isFunction(constraintRhs) &&
    !_.isArray(constraintRhs)
  ) {
    //  ┬ ┬┌─┐┌┐┌┌┬┐┬  ┌─┐  ┌─┐┌┬┐┌─┐┌┬┐┬ ┬  ┌┬┐┬┌─┐┌┬┐┬┌─┐┌┐┌┌─┐┬─┐┬ ┬
    //  ├─┤├─┤│││ │││  ├┤   ├┤ │││├─┘ │ └┬┘   ││││   │ ││ ││││├─┤├┬┘└┬┘
    //  ┴ ┴┴ ┴┘└┘─┴┘┴─┘└─┘  └─┘┴ ┴┴   ┴  ┴   ─┴┘┴└─┘ ┴ ┴└─┘┘└┘┴ ┴┴└─ ┴
    ...
    if (numKeys === 0) {
      throw flaverr(
        'E_CONSTRAINT_NOT_USABLE',
        new Error(
          'If specifying a complex constraint, there should always be at least one modifier.  But the constraint provided as `' +
            constraintTarget +
            '` has no keys-- it is just `{}`, an empty dictionary (aka plain JavaScript object).'
        )
      );
    } //-•
    ...
    if (numKeys !== 1) {
      throw new Error(
        'Consistency violation: If provided as a dictionary, the constraint RHS passed in to the internal normalizeConstraint() utility must always have exactly one key.  (Should have been normalized already.)  But instead, got: ' +
          util.inspect(constraintRhs, { depth: 5 }) +
          ''
      );
    }
    ...
    if (!MODIFIER_KINDS[modifierKind] && MODIFIER_ALIASES[modifierKind]) {
      ...
      console.warn(
        'Deprecated: The `where` clause of this query contains ' +
          '\n' +
          'a `' +
          originalModifierKind +
          '` modifier (for `' +
          constraintTarget +
          '`).  But as of Sails v1.0,' +
          '\n' +
          'this modifier is deprecated.  (Please use `' +
          modifierKind +
          '` instead.)\n' +
    ...
    //  ║║║║ ║ ║   ║╣ ║═╬╗║ ║╠═╣║
    //  ╝╚╝╚═╝ ╩   ╚═╝╚═╝╚╚═╝╩ ╩╩═╝
    if (modifierKind === '!=' || modifierKind === '$ne') {
      // Ensure this modifier is valid, normalizing it if possible.
      try {
        modifier = normalizeComparisonValue(
          modifier,
          constraintTarget,
          modelIdentity,
          orm
        ...
        switch (e.code) {
          case 'E_VALUE_NOT_USABLE':
            throw flaverr(
              'E_CONSTRAINT_NOT_USABLE',
              new Error('Invalid `!=` ("not equal") modifier.  ' + e.message)
            );
          default:
            throw e;
        }
    ...
    // between
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    else if (modifierKind === 'between' || modifierKind === '$between') {
      if (!_.isArray(modifier)) {
        throw flaverr(
          'E_CONSTRAINT_NOT_USABLE',
          new Error(
            'An `in` modifier should always be provided as an array.  ' +
              'But instead, for the `in` modifier at `' +
              constraintTarget +
              '`, got: ' +
      ...
      _.remove(modifier, function(item) {
      ...
      if (modifier.length !== 2) {
        throw flaverr(
          'E_CONSTRAINT_WOULD_MATCH_NOTHING',
          new Error(
      ...
      modifier = _.map(modifier, function(item) {
        ...
        if (_.isNull(item)) {
          throw flaverr(
            'E_CONSTRAINT_NOT_USABLE',
            new Error(
              'Got unsupported value (`null`) in an `in` modifier array.  Please use `or: [{ ' +
                constraintTarget +
                ': null }, ...]` instead.'
            )
          );
          ...
          switch (e.code) {
            case 'E_VALUE_NOT_USABLE':
              throw flaverr(
                'E_CONSTRAINT_NOT_USABLE',
                new Error(
                  'Invalid item within `in` modifier array.  ' + e.message
                )
              );
            default:
              throw e;
    ...
    //  ║║║║
    //  ╩╝╚╝
    else if (modifierKind === 'in' || modifierKind === '$in') {
      if (!_.isArray(modifier)) {
        throw flaverr(
          'E_CONSTRAINT_NOT_USABLE',
          new Error(
    ...
    //  ║║║║║║║
    //  ╝╚╝╩╝╚╝
    else if (modifierKind === 'nin' || modifierKind === '$nin') {
      if (!_.isArray(modifier)) {
        throw flaverr(
          'E_CONSTRAINT_NOT_USABLE',
          new Error(
            'A `nin` ("not in") modifier should always be provided as an array.  ' +
              'But instead, for the `nin` modifier at `' +
              constraintTarget +
              '`, got: ' +
              util.inspect(modifier, { depth: 5 }) +
              ''
      ...
      if (modifier.length === 0) {
        throw flaverr(
          'E_CONSTRAINT_WOULD_MATCH_EVERYTHING',
          new Error(
      ...
      modifier = _.map(modifier, function(item) {
        ...
        if (_.isNull(item)) {
          throw flaverr(
            'E_CONSTRAINT_NOT_USABLE',
            new Error(
              'Got unsupported value (`null`) in a `nin` ("not in") modifier array.  Please use `or: [{ ' +
                constraintTarget +
                ": { '!=': null }, ...]` instead."
            )
          );
          ...
          switch (e.code) {
            case 'E_VALUE_NOT_USABLE':
              throw flaverr(
                'E_CONSTRAINT_NOT_USABLE',
                new Error(
                  'Invalid item within `nin` ("not in") modifier array.  ' +
                    e.message
                )
              );
            default:
    ...
    //  ╚═╝╩╚═╚═╝╩ ╩ ╩ ╚═╝╩╚═   ╩ ╩ ╩╩ ╩╝╚╝
    // `>` ("greater than")
    else if (modifierKind === '>' || modifierKind === '$gt') {
      ...
      try {
        ...
        switch (e.code) {
          case 'E_VALUE_NOT_USABLE':
            throw flaverr(
              'E_CONSTRAINT_NOT_USABLE',
              new Error('Invalid `>` ("greater than") modifier.  ' + e.message)
            );
          default:
            throw e;
        }
    ...
    else if (modifierKind === '>=' || modifierKind === '$gte') {
      // If it matches a known attribute, verify that the attribute does not declare
      // itself `type: 'boolean'` (it wouldn't make any sense to attempt that)
      if (attrDef && attrDef.type === 'boolean') {
        throw flaverr(
        ...
        switch (e.code) {
          case 'E_VALUE_NOT_USABLE':
            throw flaverr(
              'E_CONSTRAINT_NOT_USABLE',
              new Error(
                'Invalid `>=` ("greater than or equal") modifier.  ' + e.message
              )
            );
          default:
            throw e;
    ...
    //  ╩═╝╚═╝╚═╝╚═╝   ╩ ╩ ╩╩ ╩╝╚╝
    // `<` ("less than")
    else if (modifierKind === '<' || modifierKind === '$lt') {
      // If it matches a known attribute, verify that the attribute does not declare
      // itself `type: 'boolean'` (it wouldn't make any sense to attempt that)
      if (attrDef && attrDef.type === 'boolean') {
        throw flaverr(
      ...
      try {
        ...
        switch (e.code) {
          case 'E_VALUE_NOT_USABLE':
            throw flaverr(
              'E_CONSTRAINT_NOT_USABLE',
              new Error('Invalid `<` ("less than") modifier.  ' + e.message)
            );
          default:
            throw e;
        }
    ...
    //  ╩═╝╚═╝╚═╝╚═╝   ╩ ╩ ╩╩ ╩╝╚╝  ╚═╝╩╚═  ╚═╝╚═╝╚╚═╝╩ ╩╩═╝
    // `<=` ("less than or equal")
    else if (modifierKind === '<=' || modifierKind === '$lte') {
      // If it matches a known attribute, verify that the attribute does not declare
      // itself `type: 'boolean'` (it wouldn't make any sense to attempt that)
      if (attrDef && attrDef.type === 'boolean') {
        throw flaverr(
        ...
        switch (e.code) {
          case 'E_VALUE_NOT_USABLE':
            throw flaverr(
              'E_CONSTRAINT_NOT_USABLE',
              new Error(
                'Invalid `<=` ("less than or equal") modifier.  ' + e.message
              )
            );
          default:
            throw e;
    ...
    //  ║  ║ ║║║║ ║ ╠═╣║║║║╚═╗
    //  ╚═╝╚═╝╝╚╝ ╩ ╩ ╩╩╝╚╝╚═╝
    else if (modifierKind === 'contains' || modifierKind === '$contains') {
    ...
    //  ╚═╗ ║ ╠═╣╠╦╝ ║ ╚═╗  ║║║║ ║ ╠═╣
    //  ╚═╝ ╩ ╩ ╩╩╚═ ╩ ╚═╝  ╚╩╝╩ ╩ ╩ ╩
    else if (modifierKind === 'startsWith' || modifierKind === '$startsWith') {
      // If it matches a known attribute, verify that the attribute
      // does not declare itself `type: 'boolean'` or `type: 'number'`;
      // and also, if it is a singular association, that the associated
      // model's primary key value is not a number either.
    ...
    //  ║╣ ║║║ ║║╚═╗  ║║║║ ║ ╠═╣
    //  ╚═╝╝╚╝═╩╝╚═╝  ╚╩╝╩ ╩ ╩ ╩
    else if (modifierKind === 'endsWith' || modifierKind === '$endsWith') {
      // If it matches a known attribute, verify that the attribute
      // does not declare itself `type: 'boolean'` or `type: 'number'`;
      // and also, if it is a singular association, that the associated
      // model's primary key value is not a number either.
    ...
    //  ║  ║╠╩╗║╣
    //  ╩═╝╩╩ ╩╚═╝
    else if (modifierKind === 'like' || modifierKind === '$like') {
      // If it matches a known attribute, verify that the attribute
      // does not declare itself `type: 'boolean'` or `type: 'number'`;
      // and also, if it is a singular association, that the associated
      // model's primary key value is not a number either.
    ...
    else if (modifierKind === 'has' || modifierKind === '$has') {
      if (
        !(_.isString(modifier) || _.isNumber(modifier) || _.isBoolean(modifier))
      ) {
        throw flaverr(
          'E_CONSTRAINT_NOT_USABLE',
          new Error(
            '\n\nAn `in` modifier should always be provided as an string, number or boolean.  ' +
              'But instead, for the `in` modifier at `' +
              constraintTarget +
              '`, got: ' +
              util.inspect(modifier, { depth: 5 }) +
    ...
    // A complex constraint must always contain a recognized modifier.
      ...
      throw flaverr(
        'E_CONSTRAINT_NOT_USABLE',
        new Error(
          'Unrecognized modifier Provided (`' +
            modifierKind +
            '`) within provided constraint for `' +
            constraintTarget +
            '`.'
        )
      );
  ...
  // > If there is no attr def, then check that it's a string, number, boolean, or `null`.
    ...
    try {
      constraintRhs = normalizeComparisonValue(
        constraintRhs,
        constraintTarget,
        modelIdentity,
        orm
      ...
      switch (e.code) {
        case 'E_VALUE_NOT_USABLE':
          throw flaverr('E_CONSTRAINT_NOT_USABLE', e);
        default:
          throw e;
      }
    } //>-•
