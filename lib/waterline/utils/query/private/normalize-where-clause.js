...
// Predicate operators
var PREDICATE_OPERATOR_KINDS = ['$or', 'or', 'and', '$and'];
 ...
 */
module.exports = function normalizeWhereClause(
  whereClause,
  modelIdentity,
  orm,
  meta
  ...
  if (_.isNull(whereClause)) {
    ...
    console.warn(
      'Deprecated: In previous versions of Waterline, the specified `where` ' +
        '\n' +
        'clause (`null`) would match ALL records in this model (`' +
        modelIdentity +
        '`).  ' +
        '\n' +
  ...
  if (
    ...
    _.isNumber(whereClause) ||
    _.isString(whereClause)
  ) {
  ...
  if (
    !_.isObject(whereClause) ||
    _.isArray(whereClause) ||
    _.isFunction(whereClause)
  ) {
    throw flaverr(
      'E_WHERE_CLAUSE_UNUSABLE',
      new Error(
    ...
    (function _recursiveStep(branch, recursionDepth, parent, indexInParent) {
      ...
      if (recursionDepth > MAX_RECURSION_DEPTH) {
        throw flaverr(
          'E_WHERE_CLAUSE_UNUSABLE',
          new Error(
            'This `where` clause seems to have a circular reference. Aborted automatically after reaching maximum recursion depth (' +
              MAX_RECURSION_DEPTH +
              ').'
          )
        );
      ...
      if (!_.contains(PREDICATE_OPERATOR_KINDS, soleBranchKey)) {
        ...
        var isComplexConstraint =
          _.isObject(branch[soleBranchKey]) &&
          !_.isArray(branch[soleBranchKey]) &&
          !_.isFunction(branch[soleBranchKey]);
        ...
        // Otherwise, we can go ahead and normalize the constraint, then bail.
          ...
          try {
            branch[soleBranchKey] = normalizeConstraint(
              branch[soleBranchKey],
              soleBranchKey,
              modelIdentity,
              orm,
              meta
      ...
      assert(
        soleBranchKey === 'and' ||
          ...
          soleBranchKey === 'or' ||
          soleBranchKey === '$or',
        'Should never have made it here if the sole branch key is not `and` or `or`!'
      );
        ...
        throw flaverr(
          ...
          new Error(
            'Expected an array at `' +
              soleBranchKey +
              '`, but instead got: ' +
              util.inspect(conjunctsOrDisjuncts, { depth: 5 }) +
      ...
      _.each(conjunctsOrDisjuncts, function(conjunctOrDisjunct, i) {
        ...
        if (
          !_.isObject(conjunctOrDisjunct) ||
          _.isArray(conjunctOrDisjunct) ||
          _.isFunction(conjunctOrDisjunct)
        ) {
          throw flaverr(
            'E_WHERE_CLAUSE_UNUSABLE',
            new Error(
              'Expected each item within an `and`/`or` predicate\'s array to be a dictionary (plain JavaScript object).  But instead, got: `' +
                util.inspect(conjunctOrDisjunct, { depth: 5 }) +
                '`'
            )
          );
        }
        ...
        try {
          ...
          switch (e.code) {
            ...
            case 'E_UNIVERSAL':
              ...
              // x ∪ Ξ = Ξ
              // ```
              if (soleBranchKey === 'or' || soleBranchKey === '$or') {
                throw e;
              } //-•
            ...
            case 'E_VOID':
              ...
              // x ∩ Ø = Ø
              // ```
              if (soleBranchKey === 'and' || soleBranchKey === '$and') {
                throw e;
              } //-•
      ...
      if (soleBranchKey === 'and') {
        throw flaverr(
          'E_UNIVERSAL',
          new Error('`{and: []}` with an empty array would match everything.')
        );
      }
      ...
      // ```
        ...
        throw flaverr(
          'E_VOID',
          new Error('`{or: []}` with an empty array would match nothing.')
        );
      }
