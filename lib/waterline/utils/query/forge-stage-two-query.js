...
var isSafeNaturalNumber = require('./private/is-safe-natural-number');
...
module.exports = function forgeStageTwoQuery(query, orm) {
  ...
  if (!_.isString(query.using) || query.using === '') {
    throw new Error(
      'Consistency violation: Every stage 1 query should include a property called `using` as a non-empty string.' +
        '  But instead, got: ' +
        util.inspect(query.using, { depth: 5 })
    );
  } //-•
  ...
  try {
    ...
    switch (e.code) {
      case 'E_MODEL_NOT_REGISTERED':
        throw new Error(
          'Consistency violation: The specified `using` ("' +
            query.using +
            '") does not match the identity of any registered model.'
        );
      default:
        throw e;
    }
  ...
  if (!_.isString(query.method) || query.method === '') {
    throw new Error(
      'Consistency violation: Every stage 1 query should include a property called `method` as a non-empty string.' +
        '  But instead, got: ' +
        util.inspect(query.method, { depth: 5 })
    );
  } //-•
  ...
  var queryKeys = (function _getQueryKeys() {
    switch (query.method) {
      ...
      case 'findOne':
        return ['criteria'];
      case 'stream':
        return ['criteria', 'eachRecordFn', 'eachBatchFn'];
      case 'count':
        return ['criteria'];
      case 'sample':
        return ['criteria'];
      case 'sum':
        return ['numericAttrName', 'criteria'];
      case 'avg':
        return ['numericAttrName', 'criteria'];
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      //This methods are custom. Added for use with graph databases
      case 'createEdge':
        return ['newRecord'];
      case 'getInboundVertices':
        return ['edgeCollections', 'criteria'];
      case 'getOutboundVertices':
        return ['edgeCollections', 'criteria'];
      case 'findNear':
        return ['distanceCriteria', 'criteria'];
      case 'upsert':
        return ['criteria', 'valuesToSet'];
      case 'aggregate':
      ...
      case 'addToCollection':
        return ['targetRecordIds', 'collectionAttrName', 'associatedIds'];
      ...
      default:
        throw new Error(
          'Consistency violation: Unrecognized `method` ("' +
            query.method +
            '")'
        );
    }
  ...
  var allowedKeys = ['meta', 'using', 'method'].concat(queryKeys);
  ...
  if (extraneousKeys.length > 0) {
    throw new Error(
      'Consistency violation: Provided "stage 1 query" contains extraneous top-level keys: ' +
        extraneousKeys
    );
  }
  ...
  if (!_.isUndefined(query.meta)) {
    if (
      !_.isObject(query.meta) ||
      _.isArray(query.meta) ||
      _.isFunction(query.meta)
    ) {
      throw buildUsageError(
        'E_INVALID_META',
        'If `meta` is provided, it should be a dictionary (i.e. a plain JavaScript object).  ' +
          'But instead, got: ' +
          util.inspect(query.meta, { depth: 5 }) +
          '',
        query.using
      );
    } //-•
  ...
  if (query.method === 'destroy' && !_.isUndefined(WLModel.cascadeOnDestroy)) {
    if (!_.isBoolean(WLModel.cascadeOnDestroy)) {
      throw new Error(
        'Consistency violation: If specified, expecting `cascadeOnDestroy` model setting to be `true` or `false`.  But instead, got: ' +
          util.inspect(WLModel.cascadeOnDestroy, { depth: 5 }) +
          ''
      );
    }
  ...
  if (
    query.method === 'update' &&
    !_.isUndefined(WLModel.fetchRecordsOnUpdate)
  ) {
    if (!_.isBoolean(WLModel.fetchRecordsOnUpdate)) {
      throw new Error(
        'Consistency violation: If specified, expecting `fetchRecordsOnUpdate` model setting to be `true` or `false`.  But instead, got: ' +
          util.inspect(WLModel.fetchRecordsOnUpdate, { depth: 5 }) +
          ''
      );
    }
  ...
  if (
    query.method === 'upsert' &&
    !_.isUndefined(WLModel.fetchRecordsOnUpsert)
  ) {
    if (!_.isBoolean(WLModel.fetchRecordsOnUpsert)) {
      throw new Error(
        'Consistency violation: If specified, expecting `fetchRecordsOnUpsert` model setting to be `true` or `false`.  But instead, got: ' +
          util.inspect(WLModel.fetchRecordsOnUpsert, { depth: 5 }) +
          ''
      );
    }
    ...
    if (!query.meta || query.meta.fetch === undefined) {
      // Only bother setting the `fetch` meta key if the model setting is `true`.
      // (because otherwise it's `false`, which is the default anyway)
      if (WLModel.fetchRecordsOnUpsert) {
        query.meta = query.meta || {};
        query.meta.fetch = WLModel.fetchRecordsOnUpsert;
      }
    } //ﬁ
  } //>-
  ...
  if (
    ...
    !_.isUndefined(WLModel.fetchRecordsOnDestroy)
  ) {
    if (!_.isBoolean(WLModel.fetchRecordsOnDestroy)) {
      throw new Error(
        'Consistency violation: If specified, expecting `fetchRecordsOnDestroy` model setting to be `true` or `false`.  But instead, got: ' +
          util.inspect(WLModel.fetchRecordsOnDestroy, { depth: 5 }) +
          ''
      );
    }
  ...
  if (
    query.method === 'create' &&
    !_.isUndefined(WLModel.fetchRecordsOnCreate)
  ) {
    if (!_.isBoolean(WLModel.fetchRecordsOnCreate)) {
      throw new Error(
        'Consistency violation: If specified, expecting `fetchRecordsOnCreate` model setting to be `true` or `false`.  But instead, got: ' +
          util.inspect(WLModel.fetchRecordsOnCreate, { depth: 5 }) +
          ''
      );
    }
  ...
  //  └  └─┘ ┴ └─┘┴ ┴  ┴└─└─┘└─┘└─┘┴└──┴┘└─┘  └─┘┘└┘  └─┘┴└─└─┘┴ ┴ ┴ └─┘ o
  if (
    query.method === 'createEdge' &&
    !_.isUndefined(WLModel.fetchRecordsOnCreateEdge)
  ) {
    if (!_.isBoolean(WLModel.fetchRecordsOnCreateEdge)) {
      throw new Error(
        'Consistency violation: If specified, expecting `fetchRecordsOnCreateEdge` model setting to be `true` or `false`.  But instead, got: ' +
          util.inspect(WLModel.fetchRecordsOnCreateEdge, { depth: 5 }) +
          ''
      );
    }
    ...
    if (!query.meta || query.meta.fetch === undefined) {
      // Only bother setting the `fetch` meta key if the model setting is `true`.
      // (because otherwise it's `false`, which is the default anyway)
      if (WLModel.fetchRecordsOnCreateEdge) {
        query.meta = query.meta || {};
        query.meta.fetch = WLModel.fetchRecordsOnCreateEdge;
      }
    } //ﬁ
  } //>-
  ...
  //  └  └─┘ ┴ └─┘┴ ┴  ┴└─└─┘└─┘└─┘┴└──┴┘└─┘  └─┘┘└┘  └─┘┴└─└─┘┴ ┴ ┴ └─┘  └─┘┴ ┴└─┘┴ ┴ o
  if (
    query.method === 'createEach' &&
    !_.isUndefined(WLModel.fetchRecordsOnCreateEach)
  ) {
    if (!_.isBoolean(WLModel.fetchRecordsOnCreateEach)) {
      throw new Error(
        'Consistency violation: If specified, expecting `fetchRecordsOnCreateEach` model setting to be `true` or `false`.  But instead, got: ' +
          util.inspect(WLModel.fetchRecordsOnCreateEach, { depth: 5 }) +
          ''
      );
    }
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  (function() {
    var modelsNotUsingObjectIds = _.reduce(
      orm.collections,
      function(memo, WLModel) {
        if (WLModel.dontUseObjectIds === true) {
          memo.push(WLModel.identity);
        }
        return memo;
      },
      []
  ...
  if (query.meta !== undefined) {
    ...
    if (query.meta.fetch !== undefined) {
      ...
      if (
        _.contains(
          ['findOrCreate', 'updateOne', 'destroyOne', 'archiveOne'],
          query.method
        )
      ) {
        console.warn(
          'warn: `fetch` is unnecessary when calling .' +
            query.method +
            '().  ' +
            'If successful, .' +
            query.method +
            '() *always* returns the affected record.'
        );
      } //ﬁ
    } //ﬁ
    ...
    if (query.meta.batchSize !== undefined) {
      if (
        !_.isNumber(query.meta.batchSize) ||
        !isSafeNaturalNumber(query.meta.batchSize)
      ) {
        throw buildUsageError(
          'E_INVALID_META',
          'If provided, `batchSize` should be a whole, positive, safe, and natural integer.  ' +
            'Instead, got ' +
            util.inspect(query.meta.batchSize, { depth: null }) +
            '.',
          query.using
        );
      } //•
  ...
  if (_.contains(queryKeys, 'criteria')) {
    ...
    if (
      _.isObject(query.criteria) &&
      !_.isArray(query.criteria) &&
      (!_.isUndefined(query.criteria.select) ||
        !_.isUndefined(query.criteria.omit))
    ) {
      var PROJECTION_COMPATIBLE_METHODS = [
        'find',
        'findOne',
        'stream',
        'getOutboundVertices',
      ...
      var isCompatibleWithProjections = _.contains(
        PROJECTION_COMPATIBLE_METHODS,
        query.method
      );
      if (!isCompatibleWithProjections) {
        throw buildUsageError(
          'E_INVALID_CRITERIA',
          'Cannot use `select`/`omit` with this method (`' +
            query.method +
            '`).',
          query.using
        );
    ...
    if (
      _.isObject(query.criteria) &&
      !_.isArray(query.criteria) &&
      (!_.isUndefined(query.criteria.limit) ||
        !_.isUndefined(query.criteria.skip) ||
        !_.isUndefined(query.criteria.sort))
    ) {
      var PAGINATION_COMPATIBLE_METHODS = [
        'find',
        'stream',
        'getOutboundVertices',
        'getInboundVertices',
      ...
      var isCompatibleWithLimit = _.contains(
        PAGINATION_COMPATIBLE_METHODS,
        query.method
      );
      if (!isCompatibleWithLimit) {
        throw buildUsageError(
          'E_INVALID_CRITERIA',
          'Cannot use `limit`, `skip`, or `sort` with this method (`' +
            query.method +
            '`).',
          query.using
        );
    ...
    if (
      _.isUndefined(query.criteria) &&
      (query.method === 'update' ||
        query.method === 'destroy' ||
        query.method === 'archive')
    ) {
      throw buildUsageError(
        'E_INVALID_CRITERIA',
        'Cannot use this method (`' +
          query.method +
          '`) with a criteria of `undefined`.  (This is just a simple failsafe to help protect your data: if you really want to ' +
          query.method +
          ' ALL records, no problem-- please just be explicit and provide a criteria of `{}`.)',
        query.using
      );
    } //>-•
    ...
    try {
      if (query.method === 'sample' && !query.criteria.limit) {
        query.criteria.limit = 1;
      }
      query.criteria = normalizeCriteria(
        query.criteria,
        query.using,
        orm,
        query.meta
      ...
      switch (e.code) {
        ...
        case 'E_WOULD_RESULT_IN_NOTHING':
          throw buildUsageError(
            'E_NOOP',
            'The provided criteria would not match any records.  ' + e.message,
            query.using
          );
        ...
        default:
          throw new Error(
            'Consistency violation: Encountered unexpected internal error when attempting to normalize/validate the provided criteria:\n```\n' +
    ...
    if (
      _.contains(
        ['findOne', 'updateOne', 'destroyOne', 'archiveOne'],
        query.method
      ) &&
      _.isEqual(query.criteria.where, {})
    ) {
      throw buildUsageError(
        'E_INVALID_CRITERIA',
        'Cannot `' +
          query.method +
          '()` without specifying a more specific `where` clause (the provided `where` clause, `{}`, is too broad).' +
          (query.method === 'findOne'
            ? '  (If you want to work around this, use `.find().limit(1)`.)'
            : ''),
        query.using
      );
    } //>-•
  ...
  if (_.contains(queryKeys, 'populates')) {
    ...
    if (
      !_.isObject(query.populates) ||
      _.isArray(query.populates) ||
      _.isFunction(query.populates)
    ) {
      throw buildUsageError(
        'E_INVALID_POPULATES',
        '`populates` must be a dictionary.  But instead, got: ' +
          util.inspect(query.populates, { depth: 1 }),
        query.using
      );
    } //-•
    ...
    _.each(_.keys(query.populates), function(populateAttrName) {
      ...
      if (
        ...
        _.isUndefined(query.populates[populateAttrName])
      ) {
      ...
      try {
        ...
        switch (e.code) {
          ...
          default:
            throw new Error(
              'Consistency violation: When attempting to populate `' +
                populateAttrName +
                '` for this model (`' +
                query.using +
                "`), an unexpected error occurred looking up the association's definition.  This SHOULD never happen.  Here is the original error:\n```\n" +
                e.stack +
                '\n```'
            );
        }
      ...
      // > We know that the primary criteria has been normalized already at this point.
      if (
        query.criteria.select[0] !== '*' &&
        _.contains(query.criteria.select, populateAttrName)
      ) {
      ...
      //
      // > We know that the primary criteria has been normalized already at this point.
      var isMentionedInPrimarySort = _.any(query.criteria.sort, function(
        comparatorDirective
      ) {
      ...
      // validate and fully-normalize the provided subcriteria.
        ...
        try {
          query.populates[populateAttrName] = normalizeCriteria(
            query.populates[populateAttrName],
            otherModelIdentity,
            orm,
            query.meta
          ...
          switch (e.code) {
            case 'E_HIGHLY_IRREGULAR':
              throw buildUsageError(
                'E_INVALID_POPULATES',
                'Could not use the specified subcriteria for populating `' +
                  populateAttrName +
                  '`: ' +
                  e.message,
            ...
            default:
              throw new Error(
                'Consistency violation: Encountered unexpected internal error when attempting to normalize/validate the provided criteria for populating `' +
                  populateAttrName +
                  '`:\n```\n' +
                  util.inspect(query.populates[populateAttrName], {
                    depth: 5
                  }) +
        ...
        var isAssociationFullyCapable = isCapableOfOptimizedPopulate(
          populateAttrName,
          query.using,
          orm
        );
            ...
            if (process.env.NODE_ENV === 'production') {
              console.warn(
                '\n' +
                  ...
                  populateAttrName +
                  '` with the specified subcriteria,\n' +
  ...
  function validateNumericAttrName(attrName) {
    ...
    if (!_.isString(attrName)) {
      throw buildUsageError(
        'E_INVALID_NUMERIC_ATTR_NAME',
        'Instead of a string, got: ' + util.inspect(attrName, { depth: 5 }),
        query.using
      );
    }
    ...
    var numericAttrDef;
    try {
      numericAttrDef = getAttribute(attrName, query.using, orm);
    } catch (e) {
      switch (e.code) {
        case 'E_ATTR_NOT_REGISTERED':
          throw buildUsageError(
    ...
    var isSingularAssociationToModelWithNumericPk =
      numericAttrDef.model &&
      getAttribute(
        getModel(numericAttrDef.model, orm).primaryKey,
        numericAttrDef.model,
        orm
      ).type === 'number';
    if (isSingularAssociationToModelWithNumericPk) {
      throw buildUsageError(
        'E_INVALID_NUMERIC_ATTR_NAME',
        'While the attribute named `' +
          attrName +
          '` defined in this model IS guaranteed ' +
    ...
    if (numericAttrDef.type !== 'number') {
      throw buildUsageError(
        ...
        'The attribute named `' +
          attrName +
          '` defined in this model is not guaranteed to be a number ' +
  ...
  if (_.contains(queryKeys, 'numericAttrName')) {
    var attrName = query.numericAttrName;
    ...
    if (Array.isArray(attrName)) {
      if (attrName.length < 2) {
        throw buildUsageError(
          ...
          'The attribute named `' +
            attrName +
            '` must be an array of more than one item ' +
            "(and items should declare `type: 'number'`).",
          query.using
        );
      }
      _.each(attrName, function(attr) {
        validateNumericAttrName(attr);
      });
    } else {
      validateNumericAttrName(attrName);
    }
  } //>-•

  function validateEdgeCollection(edges) {
    var models = [];
    _.each(orm.collections, (value, key) => {
      models.push({ name: key, classType: value.classType });
    });

    const edgeCollections = models
      .filter(m => m.classType === 'Edge')
      .map(m => m.name);

    _.each(edges, edge => {
      var valid = _.includes(edgeCollections, edge);
      if (!_.isString(edge) || !valid) {
        throw buildUsageError(
          'E_INVALID_EDGE_NAME',
          'Instead of a string and a valid edge, got: ' +
            util.inspect(edge, { depth: 5 }),
          query.using
        );
      }
    });
  ...
  if (_.contains(queryKeys, 'edgeCollections')) {
    var edges = query.edgeCollections;

    if (_.isString(edges)) {
      validateEdgeCollection([edges]);
    }
    ...
    if (!_.isArray(edges) && !_.isString(edges)) {
      throw buildUsageError(
        'E_INVALID_EDGE_NAME',
        'Instead of a string or array, got: ' +
          util.inspect(edges, { depth: 5 }),
        query.using
      );
    }
  } //>-•

  if (_.contains(queryKeys, 'distanceCriteria')) {
    var criteria = {};
    let attrName;

    _.each(query.distanceCriteria || {}, (value, key) => {
      if (key !== 'radius') {
        criteria = value;
        attrName = key;
      }

      if (key === 'radius' && !_.isNumber(value)) {
        throw flaverr(
          'E_INVALID_RADIUS_ATTRIBUTE',
          new Error(
            '\n\nthe radius attribute must be of lof type `number` got: ' +
              util.inspect(center, { depth: 5 }) +
              ' ' +
              query.using +
              '\n\n'
          )
    ...
    if (!_.isNumber(criteria.latitude) || !_.isNumber(criteria.longitude)) {
      throw flaverr(
        'E_INVALID_LOCATION',
        new Error(
          '\n\nthe first attribute must be of latitude, longitude attributes, got: ' +
            util.inspect(criteria, { depth: 5 }) +
            ' ' +
            query.using +
            '\n\n'
        )
    ...
    //Check if attribute name is registered
    var numericAttrDef;
    try {
      numericAttrDef = getAttribute(attrName, query.using, orm);
    } catch (e) {
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // Check aggregateCriteria
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  if (_.contains(queryKeys, 'aggregateCriteria')) {
    //
  }
  ...
  if (
    _.contains(queryKeys, 'eachRecordFn') ||
    _.contains(queryKeys, 'eachBatchFn')
  ) {
    // -> Both functions were defined
    if (
      !_.isUndefined(query.eachRecordFn) &&
      !_.isUndefined(query.eachBatchFn)
    ) {
      throw buildUsageError(
    ...
    // -> Only `eachRecordFn` was defined
      ...
      if (!_.isFunction(query.eachRecordFn)) {
        throw buildUsageError(
          'E_INVALID_STREAM_ITERATEE',
          'For `eachRecordFn`, instead of a function, got: ' +
            util.inspect(query.eachRecordFn, { depth: 5 }),
          query.using
        );
      }
    }
    // -> Only `eachBatchFn` was defined
      ...
      if (!_.isFunction(query.eachBatchFn)) {
        throw buildUsageError(
          'E_INVALID_STREAM_ITERATEE',
          'For `eachBatchFn`, instead of a function, got: ' +
            util.inspect(query.eachBatchFn, { depth: 5 }),
          query.using
        );
      }
    }
  ...
  if (_.contains(queryKeys, 'newRecord')) {
    ...
    try {
      query.newRecord = normalizeNewRecord(
        query.newRecord,
        query.using,
        orm,
        theMomentBeforeFS2Q,
  ...
  if (_.contains(queryKeys, 'newRecords')) {
    ...
    if (!_.isArray(query.newRecords)) {
      throw buildUsageError(
        ...
        'Expecting an array but instead, got: ' +
          util.inspect(query.newRecords, { depth: 5 }),
        query.using
      );
    } //-•
    ...
    if (query.newRecords.length === 0) {
      throw buildUsageError(
        'E_NOOP',
        'No things to create were provided.',
        query.using
      );
    } //-•
  ...
  if (_.contains(queryKeys, 'valuesToSet')) {
    if (
      !_.isObject(query.valuesToSet) ||
      _.isFunction(query.valuesToSet) ||
      _.isArray(query.valuesToSet)
    ) {
      throw buildUsageError(
        'E_INVALID_VALUES_TO_SET',
        'Expecting a dictionary (plain JavaScript object) but instead, got: ' +
          util.inspect(query.valuesToSet, { depth: 5 }),
        query.using
      );
    } //-•
    ...
    _.each(_.keys(query.valuesToSet), function(attrNameToSet) {
      ...
      try {
        query.valuesToSet[attrNameToSet] = normalizeValueToSet(
          query.valuesToSet[attrNameToSet],
          attrNameToSet,
          query.using,
          orm,
          query.meta
    ...
    _.each(WLModel.attributes, function(attrDef, attrName) {
      if (!attrDef.autoUpdatedAt) {
        return;
      }
      if (!_.isUndefined(query.valuesToSet[attrName])) {
        return;
      }
      ...
      assert(
        attrDef.type === 'number' ||
          attrDef.type === 'string' ||
          attrDef.type === 'ref',
        "If an attribute has `autoUpdatedAt: true`, then it should always have either `type: 'string'`, `type: 'number'` or `type: 'ref'`.  But the definition for attribute (`" +
          attrName +
          "`) has somehow gotten into this state!  This should be impossible, but it has both `autoUpdatedAt: true` AND `type: '" +
          attrDef.type +
          "'`"
      );
  ...
  if (_.contains(queryKeys, 'collectionAttrName')) {
    if (!_.isString(query.collectionAttrName)) {
      throw buildUsageError(
        'E_INVALID_COLLECTION_ATTR_NAME',
        'Instead of a string, got: ' +
          util.inspect(query.collectionAttrName, { depth: 5 }),
        query.using
      );
    }
    ...
    if (!associationDef.collection) {
      throw buildUsageError(
        ...
        'The attribute named `' +
          query.collectionAttrName +
          '` defined in this model is not a plural ("collection") association.',
        query.using
      );
    }
  } //>-•
  ...
  if (_.contains(queryKeys, 'targetRecordIds')) {
    ...
    if (query.targetRecordIds.length === 0) {
      throw buildUsageError(
        'E_NOOP',
        'No target record ids were provided.',
        query.using
      );
    } //-•
    ...
    var isRelevantMethod =
      query.method === 'addToCollection' ||
      query.method === 'replaceCollection';
    var isTryingToSetOneOrMoreAssociatedIds =
      _.isArray(query.associatedIds) && query.associatedIds.length > 0;
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    if (
      query.targetRecordIds.length > 1 &&
      isRelevantMethod &&
      isTryingToSetOneOrMoreAssociatedIds
    ) {
      ...
      var isAssociationExclusive = isExclusive(
        query.collectionAttrName,
        query.using,
        orm
      );
      ...
      if (isAssociationExclusive) {
        throw buildUsageError(
          ...
          'The  `' +
            ...
            '` association of the `' +
            query.using +
            '` model is exclusive, meaning that associated child ' +
            'records cannot belong to the `' +
            query.collectionAttrName +
            '` collection of more than one `' +
            query.using +
            '` record.  ' +
            'You are seeing this error because this query would have tried to share the same child record(s) across the `' +
            query.collectionAttrName +
            '` ' +
            'collections of 2 or more different `' +
            query.using +
            '` records.  To resolve this error, change the query, or change your models ' +
  ...
  if (_.contains(queryKeys, 'associatedIds')) {
    ...
    var associatedPkType = (function() {
      var _associationDef = getAttribute(
        query.collectionAttrName,
        query.using,
        orm
      );
      ...
      var _associatedPkDef = getAttribute(
        AssociatedModel.primaryKey,
        _otherModelIdentity,
        orm
      );
    ...
    // > model's primary key attribute.
    try {
      query.associatedIds = normalizePkValueOrValues(
        query.associatedIds,
        associatedPkType
      );
    } catch (e) {
    ...
    var isQueryMeaningfulWithNoAssociatedIds =
      query.method === 'removeFromCollection' ||
      query.method === 'addToCollection';
    if (
      query.associatedIds.length === 0 &&
      isQueryMeaningfulWithNoAssociatedIds
    ) {
      throw buildUsageError(
        'E_NOOP',
        'No associated ids were provided.',
        query.using
      );
    } //-•
  } //>-•
