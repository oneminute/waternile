      ...
      (function _maybeRunBeforeLC(proceed) {
        ...
        adapter.createEdge(
          ...
          function _afterTalkingToAdapter(err, rawAdapterResult, options) {
            ...
            var fetch =
              modifiedMeta || (_.has(options, 'fetch') && options.fetch);
            if (!fetch) {
            ...
            if (
              ...
              return done(
                new Error(
                  'Consistency violation: expected `create` adapter method to send back the created record b/c `fetch: true` was enabled.  But instead, got: ' +
                    util.inspect(rawAdapterResult, { depth: 5 }) +
                    ''
                )
              );
