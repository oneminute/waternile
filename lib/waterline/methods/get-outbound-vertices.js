...
var DEFERRED_METHODS = getQueryModifierMethods('getOutboundVertices');

/**
 * getOutboundVertices()
 *
 * Get the aggregate getOutboundVertices of the specified attribute across all matching records.
 *
 * ```
 * // The cumulative account balance of all bank accounts that have
 * // less than $32,000, or that are flagged as "suspended".
 * BankAccount.getOutboundVertices('balance').where({
 *   or: [
 *     { balance: { '<': 32000 } },
 *     { suspended: true }
 *   ]
...
module.exports = function getOutboundVertices(/* edgeCollections?, criteria?, explicitCbMaybe?, meta?, moreQueryKeys? */) {
  ...
  var omen = buildOmen(getOutboundVertices);
  ...
  // • getOutboundVertices(edgeCollections, explicitCbMaybe, ..., ...)
  if (args.length >= 2 && _.isFunction(args[1])) {
    query.edgeCollections = args[0];
  ...
  // • getOutboundVertices(edgeCollections, criteria, ..., ..., ...)
  else {
    query.edgeCollections = args[0];
    ...
    throw flaverr(
      {
        name: 'UsageError',
        message:
          '`.getOutboundVertices()` received an explicit callback function argument... but it ' +
          'was not a function: ' +
          explicitCbMaybe,
      },
      omen
  ...
  return parley(
    function(done) {
              ...
              flaverr(
                {
                  ...
                  details: e.details,
                  message:
                    'The numeric attr name (i.e. first argument) to `.getOutboundVertices()` should ' +
      ...
      if (!adapter.getOutboundVertices) {
        return done(
          new Error(
      ...
      adapter.getOutboundVertices(
        WLModel.datastore,
        query,
        function _afterTalkingToAdapter(err, getOutboundVertices) {
          if (err) {
          ...
          // the getOutboundVertices sent back by the adapter turns out to be something other
          ...
          // ```
          // !_.isNumber(getOutboundVertices) || getOutboundVertices === Infinity || getOutboundVertices === -Infinity || _.isNaN(getOutboundVertices)
          // ````
          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

          return done(undefined, getOutboundVertices);
        }
      ); //</adapter.getOutboundVertices()>
    },
