  ...
  return parley(
    function (done) {
      ...
      (function (proceed) {
        ...
        try {
          assert.equal(
            query.using.toLowerCase(),
            query.using,
            '`query.using` (identity) should have already been normalized before getting here!  But it was not: ' +
              query.using
          );
        } catch (e) {
          return proceed(e);
        ...
        try {
          assert.equal(
            schemaDef.collection.toLowerCase(),
            schemaDef.collection,
            '`schemaDef.collection` (identity) should have already been normalized before getting here!  But it was not: ' +
              schemaDef.collection
          );
          assert.equal(
            schemaDef.referenceIdentity.toLowerCase(),
            schemaDef.referenceIdentity,
            '`schemaDef.referenceIdentity` (identity) should have already been normalized before getting here!  But it was not: ' +
              schemaDef.referenceIdentity
          );
          assert.equal(
            Object.getPrototypeOf(WLChild).identity.toLowerCase(),
            Object.getPrototypeOf(WLChild).identity,
            '`Object.getPrototypeOf(WLChild).identity` (identity) should have already been normalized before getting here!  But it was not: ' +
              Object.getPrototypeOf(WLChild).identity
          );
        } catch (e) {
          return proceed(e);
        ...
        if (
          _.has(Object.getPrototypeOf(WLChild), 'junctionTable') &&
          WLChild.junctionTable
        ) {
          manyToMany = true;
        }
        ...
        if (
          _.has(Object.getPrototypeOf(WLChild), 'throughTable') &&
          _.keys(WLChild.throughTable).length
        ) {
          manyToMany = true;
        }
        ...
        var modifiedMeta = _.extend({}, query.meta || {}, {
        ...
        if (manyToMany) {
          ...
          if (
            ...
            _.each(WLChild.schema, function (wlsAttrDef, key) {
              ...
              if (
                _.has(wlsAttrDef, 'columnName') &&
                wlsAttrDef.columnName === schemaDef.on
              ) {
                parentReference = key;
              }
            });
          ...
          // through table mapping that was generated by Waterline-Schema.
            ...
            childReference =
              WLChild.throughTable[
                WLModel.identity + '.' + query.collectionAttrName
              ];
          ...
          //  ╩╚═╚═╝╝╚╝  ─┴┘└─┘└─┘ ┴ ┴└─└─┘ ┴   └─┘└└─┘└─┘┴└─ ┴
          WLChild.destroy(
            criteriaOfDestruction,
            function $afterDestroyingChildRecords(err) {
              if (err) {
                return proceed(err);
              }
        ...
        if (
          query.targetRecordIds.length >= 2 &&
          query.associatedIds.length > 0
        ) {
          return proceed(
            new Error(
              'Consistency violation: Too many target record ids and associated ids-- should never have been possible, because this query should have been halted when it was being forged at stage 2.'
            )
          );
        }
        if (query.targetRecordIds.length === 0) {
          return proceed(
            new Error(
              'Consistency violation: No target record ids-- should never have been possible, because this query should have been halted when it was being forged at stage 2.'
            )
          );
        }
        ...
        (function (proceed) {
          ...
          potentialCollisionCriteria.where[schemaDef.via] = {
            in: query.targetRecordIds,
          };
          potentialCollisionCriteria.where[WLChild.primaryKey] = {
            nin: query.associatedIds,
          };
              ...
              flaverr(
                {
                  ...
                  message:
                    'Cannot ' +
                    (query.associatedIds.length === 0 ? 'wipe' : 'replace') +
                    ' the contents of ' +
                    'association (`' +
                    query.collectionAttrName +
                    '`) because there ' +
                    (numCollisions === 1
                      ? 'is one conflicting ' + WLChild.identity + ' record'
                      : 'are ' +
                        numCollisions +
                        ' conflicting ' +
                        WLChild.identity +
                    ...
                    'whose `' +
                    schemaDef.via +
                    '` cannot be set to `null`.  (That attribute is required.)',
          ...
          if (query.associatedIds.length > 0) {
            ...
            partialNullOutCriteria.where[WLChild.primaryKey] = {
              nin: query.associatedIds,
            };
            partialNullOutCriteria.where[schemaDef.via] =
              query.targetRecordIds[0];
            ...
            WLChild.update(
              ...
              function (err) {
                ...
                newFkUpdateCriteria.where[WLChild.primaryKey] = {
                  in: query.associatedIds,
                };
                ...
                WLChild.update(
                  ...
                  modifiedMeta
                ); //_∏_    </WLChild.update() -- the "new fk update" query>
              },
              modifiedMeta
            ); //_∏_    </WLChild.update() -- the "partial null-out" query>
          } //‡
          //  ╦═╗╦ ╦╔╗╔  ┌┐ ┬  ┌─┐┌┐┌┬┌─┌─┐┌┬┐  ┌┐┌┬ ┬┬  ┬    ┌─┐┬ ┬┌┬┐  ┌─┐ ┬ ┬┌─┐┬─┐┬ ┬
          //  ╠╦╝║ ║║║║  ├┴┐│  ├─┤│││├┴┐├┤  │   ││││ ││  │    │ ││ │ │   │─┼┐│ │├┤ ├┬┘└┬┘
          //  ╩╚═╚═╝╝╚╝  └─┘┴─┘┴ ┴┘└┘┴ ┴└─┘ ┴   ┘└┘└─┘┴─┘┴─┘  └─┘└─┘ ┴   └─┘└└─┘└─┘┴└─ ┴
          // Alternatively, we'll go with scenario B, where we potentially null all the fks out.
            ...
            WLChild.update(
              ...
              modifiedMeta
            ); //_∏_    </WLChild.update() -- the "blanket null-out" query>
          } //ﬁ
