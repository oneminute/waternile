...
var DEFERRED_METHODS = getQueryModifierMethods('upsert');
    ...
    function(done) {
      ...
      try {
        ...
        switch (e.code) {
              ...
              flaverr(
                {
                  ...
                  details: e.details,
                  message:
                    'Cannot perform upsert with the provided values.\n' +
                    'Details:\n' +
                    '  ' +
                    e.details +
                    '\n',
          ...
          case 'E_NOOP':
            ...
            // >  See: https://github.com/treelinehq/waterline-query-docs/blob/master/docs/results.md#upsert
      ...
      (function(proceed) {
        ...
        _.each(WLModel.attributes, function _eachKnownAttrDef(
          ...
          if (attrDef.collection) {
            ...
            if (query.valuesToSet[attrName]) {
              ...
              // doesn't need to do anything during the initial upsert.
        ...
        if (_.keys(collectionResets).length > 0) {
          ...
          modifiedMetaForCollectionResets = _.extend({}, query.meta || {}, {
            fetch: true,
          });
        } //>-
        ...
        adapter.upsert(
          ...
          function _afterTalkingToAdapter(err, rawAdapterResult) {
            ...
            //  └┴┘┴ ┴└─┘  └─┘└─┘ ┴    ┴ └─┘   ┴ ┴└─└─┘└─┘
            var fetch =
              modifiedMetaForCollectionResets ||
              (_.has(query.meta, 'fetch') && query.meta.fetch);
              ...
              if (!_.isUndefined(rawAdapterResult)) {
                console.warn(
                  '\n' +
                    ...
                    'from its `upsert` method.  But it did!  And since it\'s an array, displaying this\n' +
              ...
              return done(
                new Error(
                  'Unexpected behavior in database adapter: Since `fetch: true` was enabled, this adapter ' +
                    '(for datastore `' +
                    WLModel.datastore +
                    '`) should have sent back an array of records as the ' +
                    '2nd argument when triggering the callback from its `upsert` method.  But instead, got: ' +
                    util.inspect(rawAdapterResult, { depth: 5 }) +
                    ''
                )
              );
              ...
              function _afterReplacingAllCollections(err) {
                ...
                async.each(
                  ...
                  function _eachRecord(record, next) {
                    ...
                    // Otherwise run it.
                    WLModel._callbacks.afterUpdate(
                      record,
