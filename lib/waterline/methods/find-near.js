...
var DEFERRED_METHODS = getQueryModifierMethods('findNear');

/**
 * findNear()
 *
 * Get the aggregate findNear of the specified attribute across all matching records.
 *
 * ```
 * // The cumulative account balance of all bank accounts that have
 * // less than $32,000, or that are flagged as "suspended".
 * BankAccount.findNear('balance').where({
 *   or: [
 *     { balance: { '<': 32000 } },
 *     { suspended: true }
 *   ]
...
module.exports = function findNear(/* distanceCriteria?, criteria?, explicitCbMaybe?, meta?, moreQueryKeys? */) {
  ...
  // • findNear(distanceCriteria, explicitCbMaybe, ..., ...)
  if (args.length >= 2 && _.isFunction(args[1])) {
    query.distanceCriteria = args[0];
  ...
  // • findNear(distanceCriteria, criteria, ..., ..., ...)
  else {
    query.distanceCriteria = args[0];
  ...
  var distanceCriteria = query.distanceCriteria || {};

  // We must make sure the distanceCriteria is provided.

  _.each(distanceCriteria, (value, key) => {
    if (key !== 'radius') {
      if (!_.has(value, 'longitude') || !_.has(value, 'latitude')) {
        throw flaverr(
          {
            name: 'UsageError',
            message:
              '\n\n`.findNear()` must receive a valid location to find near distances ' +
              'was not a valid location with latitude & longitude: ' +
              key +
              ': ' +
    ...
    throw flaverr(
      {
        name: 'UsageError',
        message:
          '`.findNear()` received an explicit callback function argument... but it ' +
          'was not a function: ' +
          explicitCbMaybe,
      },
      omen
  ...
  return parley(
    function(done) {
              ...
              flaverr(
                {
                  ...
                  details: e.details,
                  message:
                    'The numeric attr name (i.e. first argument) to `.findNear()` should ' +
      ...
      adapter.findNear(
        WLModel.datastore,
        query,
        function _afterTalkingToAdapter(err, findNear) {
          if (err) {
            err = forgeAdapterError(err, omen, 'findNear', modelIdentity, orm);
            return done(err);
          } //-•

          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
          // FUTURE: Log a warning like the ones in `process-all-records` if
          // the findNear sent back by the adapter turns out to be something other
          ...
          // ```
          // !_.isNumber(findNear) || findNear === Infinity || findNear === -Infinity || _.isNaN(findNear)
          // ````
          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

          return done(undefined, findNear);
        }
      ); //</adapter.findNear()>
    },
