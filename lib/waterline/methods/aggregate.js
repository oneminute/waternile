...
var DEFERRED_METHODS = getQueryModifierMethods('aggregate');

/**
 * aggregate()
 *
 * Get the aggregate aggregate of the specified attribute across all matching records.
 *
 * ```
 * // The cumulative account balance of all bank accounts that have
 * // less than $32,000, or that are flagged as "suspended".
 * BankAccount.aggregate('balance').where({
 *   or: [
 *     { balance: { '<': 32000 } },
 *     { suspended: true }
 *   ]
 ...
 * ================================================
 *
 * @param {String?} aggregateCriteria
 *
...
module.exports = function aggregate(/* aggregateCriteria?, criteria?, explicitCbMaybe?, meta?, moreQueryKeys? */) {
  ...
  // • aggregate(aggregateCriteria, explicitCbMaybe, ..., ...)
  if (args.length >= 2 && _.isFunction(args[1])) {
    query.aggregateCriteria = args[0];
  ...
  // • aggregate(aggregateCriteria, criteria, ..., ..., ...)
  else {
    query.aggregateCriteria = args[0];
    ...
    throw flaverr(
      {
        name: 'UsageError',
        message:
          '`.aggregate()` received an explicit callback function argument... but it ' +
          'was not a function: ' +
          explicitCbMaybe,
      },
      omen
  ...
  return parley(
    function (done) {
              ...
              flaverr(
                {
                  ...
                  details: e.details,
                  message:
                    'The numeric attr name (i.e. first argument) to `.aggregate()` should ' +
      ...
      adapter.aggregate(
        WLModel.datastore,
        query,
        function _afterTalkingToAdapter(err, aggregate) {
          if (err) {
            err = forgeAdapterError(err, omen, 'aggregate', modelIdentity, orm);
            return done(err);
          } //-•

          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
          // FUTURE: Log a warning like the ones in `process-all-records` if
          // the aggregate sent back by the adapter turns out to be something other
          ...
          // ```
          // !_.isNumber(aggregate) || aggregate === Infinity || aggregate === -Infinity || _.isNaN(aggregate)
          // ````
          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

          return done(undefined, aggregate);
        }
      ); //</adapter.aggregate()>
    },
