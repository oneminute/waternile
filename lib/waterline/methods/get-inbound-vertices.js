...
var DEFERRED_METHODS = getQueryModifierMethods('getInboundVertices');

/**
 * getInboundVertices()
 *
 * Get the aggregate getInboundVertices of the specified attribute across all matching records.
 *
 * ```
 * // The cumulative account balance of all bank accounts that have
 * // less than $32,000, or that are flagged as "suspended".
 * BankAccount.getInboundVertices('balance').where({
 *   or: [
 *     { balance: { '<': 32000 } },
 *     { suspended: true }
 *   ]
...
module.exports = function getInboundVertices(/* edgeCollections?, criteria?, explicitCbMaybe?, meta?, moreQueryKeys? */) {
  ...
  var omen = buildOmen(getInboundVertices);
  ...
  // • getInboundVertices(edgeCollections, explicitCbMaybe, ..., ...)
  ...
  // • getInboundVertices(edgeCollections, criteria, ..., ..., ...)
  else {
    query.edgeCollections = args[0];
    ...
    throw flaverr(
      {
        name: 'UsageError',
        message:
          '`.getInboundVertices()` received an explicit callback function argument... but it ' +
          'was not a function: ' +
          explicitCbMaybe,
      },
      omen
  ...
  return parley(
    function(done) {
              ...
              flaverr(
                {
                  ...
                  details: e.details,
                  message:
                    'The numeric attr name (i.e. first argument) to `.getInboundVertices()` should ' +
      ...
      adapter.getInboundVertices(
        WLModel.datastore,
        query,
        function _afterTalkingToAdapter(err, getInboundVertices) {
          if (err) {
          ...
          // the getInboundVertices sent back by the adapter turns out to be something other
          ...
          // ```
          // !_.isNumber(getInboundVertices) || getInboundVertices === Infinity || getInboundVertices === -Infinity || _.isNaN(getInboundVertices)
          // ````
          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

          return done(undefined, getInboundVertices);
        }
      ); //</adapter.getInboundVertices()>
    },
