...
var DEFERRED_METHODS = getQueryModifierMethods('findAny');

/**
 * findAny()
 *
 * Get the aggregate findAny of the specified attribute across all matching records.
 *
 * ```
 * // The cumulative account balance of all bank accounts that have
 * // less than $32,000, or that are flagged as "suspended".
 * BankAccount.findAny('balance').where({
 *   or: [
 *     { balance: { '<': 32000 } },
 *     { suspended: true }
 *   ]
...
module.exports = function findAny(/*  criteria?, edgeCollections?, explicitCbMaybe?, meta?, moreQueryKeys? */) {
  ...
  // • findAny(criteria, edgeCollections, explicitCbMaybe,  ..., ..., ...)
    ...
    throw flaverr(
      {
        name: 'UsageError',
        message:
          '`.findAny()` received an explicit callback function argument... but it ' +
          'was not a function: ' +
          explicitCbMaybe,
      },
      omen
  ...
  return parley(
    function (done) {
              ...
              flaverr(
                {
                  ...
                  details: e.details,
                  message:
                    'The numeric attr name (i.e. first argument) to `.findAny()` should ' +
      ...
      adapter.findAny(WLModel.datastore, query, function _afterTalkingToAdapter(
        err,
        findAny
      ) {
        if (err) {
          err = forgeAdapterError(err, omen, 'findAny', modelIdentity, orm);
          return done(err);
        } //-•

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // FUTURE: Log a warning like the ones in `process-all-records` if
        // the findAny sent back by the adapter turns out to be something other
        ...
        // ```
        // !_.isNumber(findAny) || findAny === Infinity || findAny === -Infinity || _.isNaN(findAny)
        // ````
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        return done(undefined, findAny);
      }); //</adapter.findAny()>
    },
