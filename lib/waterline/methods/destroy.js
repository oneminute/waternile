    ...
    function (done) {
      ...
      (function _runBeforeLC(proceed) {
        ...
        if (!adapter) {
          ...
          return done(
            new Error(
              'Consistency violation: Cannot find adapter for model (`' +
                modelIdentity +
                '`).  This model appears to be using datastore `' +
                WLModel.datastore +
                '`, but the adapter for that datastore cannot be located.'
            )
          );
        }
          ...
          return done(
            new Error(
              'The adapter used by this model (`' +
                modelIdentity +
                "`) doesn't support the `destroy` method."
            )
          );
        }
          ...
          if (!adapter.find) {
            return done(
              new Error(
                'The adapter used by this model (`' +
                  modelIdentity +
                  "`) doesn't support the `find` method, but that method is mandatory to be able to use `cascade: true`."
              )
            );
          }
        } //>-
        ...
        (function _maybeFindIdsToDestroy(proceed) {
          ...
          if (!pkColumnName) {
            return done(
              new Error(
                'Consistency violation: model `' +
                  WLModel.identity +
                  '` schema has no primary key column name!'
              )
            );
          }
          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
          ...
          err,
          idsOfRecordsBeingDestroyedMaybe
        ) {
          if (err) {
            return done(err);
          }
          ...
          adapter.destroy(
            WLModel.datastore,
            query,
            function _afterTalkingToAdapter(err, rawAdapterResult) {
              if (err) {
              ...
              (function _maybeWipeAssociatedCollections(proceed) {
                ...
                if (!_.isArray(idsOfRecordsBeingDestroyedMaybe)) {
                  return proceed(
                    new Error(
                      'Consistency violation: Should have an array of records looked up before!  But instead, got: ' +
                        util.inspect(idsOfRecordsBeingDestroyedMaybe, {
                          depth: 5,
                        }) +
                        ''
                    )
                ...
                async.each(
                  ...
                  function _eachAttribute(attrName, next) {
                              ...
                              flaverr(
                                {
                                  ...
                                  code: err.code,
                                  message:
                                    'Failed to run the "cascade" polyfill.  Could not propagate the potential ' +
                                    'destruction of ' +
                                    (idsOfRecordsBeingDestroyedMaybe.length ===
                                    1
                                      ? 'this ' + WLModel.identity + ' record'
                                      : 'these ' +
                                        idsOfRecordsBeingDestroyedMaybe.length +
                                        ' ' +
                                        WLModel.identity +
                ...
                (function _maybeTransformRecords(proceed) {
                  ...
                  if (
                    !_.has(query.meta, 'fetch') ||
                    query.meta.fetch === false
                  ) {
                    ...
                    if (!_.isUndefined(rawAdapterResult)) {
                      console.warn(
                        '\n' +
                          ...
                          'Since `fetch` is NOT enabled, this adapter (for datastore `' +
                          WLModel.datastore +
                          '`)\n' +
                    ...
                    return proceed(
                      new Error(
                        'Unexpected behavior in database adapter: Since `fetch: true` was enabled, this adapter ' +
                          '(for datastore `' +
                          WLModel.datastore +
                          '`) should have sent back an array of records as the 2nd argument when triggering ' +
                          'the callback from its `destroy` method.  But instead, got: ' +
                          util.inspect(rawAdapterResult, { depth: 5 }) +
                          ''
                      )
                    );
                  ...
                  try {
                    transformedRecords = rawAdapterResult.map(function (
                      record
                    ) {
                  ...
                  // ============================================================
                  async.each(
                    transformedRecordsMaybe,
                    function _eachRecord(record, next) {
                      ...
                      // Otherwise run it.
                      WLModel._callbacks.afterDestroy(
                        record,
