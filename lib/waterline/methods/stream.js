...
module.exports = function stream(/* criteria?, eachRecordFn?, explicitCbMaybe?, meta?, moreQueryKeys? */) {
  ...
  return parley(
    function (done) {
      ...
      var batchSize =
        query.meta && query.meta.batchSize !== undefined
          ? query.meta.batchSize
          : DEFAULT_BATCH_SIZE;
      ...
      async.whilst(
        function _checkHasntReachedLastBatchYet() {
          if (!reachedLastBatch) {
            return true;
          } else {
            return false;
        ...
        function _beginBatchMaybe(next) {
          ...
          // 45  => 5
          // 50
          var numRecordsLeftUntilAbsLimit =
            query.criteria.limit - i * batchSize;
          var limitForThisBatch = Math.min(
          ...
          _.each(query.populates, function (assocCriteria, assocName) {
            deferredForThisBatch = deferredForThisBatch.populate(
              assocName,
              assocCriteria
            );
          });
          ...
          deferredForThisBatch.meta(
            query.meta ? _.omit(query.meta, ['batchSize']) : undefined
          );
          ...
          deferredForThisBatch.exec(function (err, batchOfRecords) {
            ...
            (function _makeCallOrCallsToAppropriateIteratee(proceed) {
              ...
              var seemsToExpectCallback = (function () {
                ...
                var parametersAsString = fnStr.slice(
                  fnStr.indexOf('(') + 1,
                  fnStr.indexOf(')')
                );
              ...
              if (query.eachBatchFn) {
                ...
                try {
                  var promiseMaybe = query.eachBatchFn(
                    batchOfRecords,
                    function (err) {
                      if (!seemsToExpectCallback) {
                        return proceed(
                          new Error(
                            'Unexpected attempt to invoke callback.  Since this per-batch iteratee function does not appear to expect a callback parameter, this stub callback was provided instead.  Please either explicitly list the callback parameter among the arguments or change this code to no longer use a callback.'
                          )
                        );
                      } //•
                      if (err) {
              ...
              async.eachSeries(
                batchOfRecords,
                function _eachRecordInBatch(record, next) {
                  // Note that, if you try to call next() more than once in the iteratee, Waterline
                  // logs a warning explaining what's up, ignoring all subsequent calls to next()
                  // that occur after the first.
                  var didIterateeAlreadyHalt;
                  try {
                    var promiseMaybe = query.eachRecordFn(record, function (
                      err
                    ) {
                      if (!seemsToExpectCallback) {
                        return next(
                          new Error(
                            'Unexpected attempt to invoke callback.  Since this per-record iteratee function does not appear to expect a callback parameter, this stub callback was provided instead.  Please either explicitly list the callback parameter among the arguments or change this code to no longer use a callback.'
                          )
                        );
                      } //•
                      if (err) {
                    ...
                    // Take care of unhandled promise rejections from `await` (if appropriate)
                    if (
                      query.eachRecordFn.constructor.name === 'AsyncFunction'
                    ) {
                      if (!seemsToExpectCallback) {
                        promiseMaybe = promiseMaybe.then(function () {
                          didIterateeAlreadyHalt = true;
