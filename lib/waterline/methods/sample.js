...
var DEFERRED_METHODS = getQueryModifierMethods('sample');
...
module.exports = function sample(/* criteria?, explicitCbMaybe?, meta?, moreQueryKeys? */) {
  ...
  return parley(
    function(done) {
      ...
      adapter.sample(WLModel.datastore, query, function _afterTalkingToAdapter(
        err,
        numRecords
      ) {
        if (err) {
          err = forgeAdapterError(err, omen, 'sample', modelIdentity, orm);
          return done(err);
        }
