...
var DEFERRED_METHODS = getQueryModifierMethods('findInbound');

/**
 * findInbound()
 *
 * Get the aggregate findInbound of the specified attribute across all matching records.
 *
 * ```
 * // The cumulative account balance of all bank accounts that have
 * // less than $32,000, or that are flagged as "suspended".
 * BankAccount.findInbound('balance').where({
 *   or: [
 *     { balance: { '<': 32000 } },
 *     { suspended: true }
 *   ]
...
module.exports = function findInbound(/* criteria?, edgeCollections?, explicitCbMaybe?, meta?, moreQueryKeys? */) {
  ...
  var omen = buildOmen(findInbound);
  ...
  // • findInbound( criteria, edgeCollections, explicitCbMaybe, ..., ..., ...)
    ...
    throw flaverr(
      {
        name: 'UsageError',
        message:
          '`.findInbound()` received an explicit callback function argument... but it ' +
          'was not a function: ' +
          explicitCbMaybe
      },
      omen
  ...
  return parley(
    function(done) {
              ...
              flaverr(
                {
                  ...
                  details: e.details,
                  message:
                    'The numeric attr name (i.e. first argument) to `.findInbound()` should ' +
      ...
      adapter.findInbound(
        WLModel.datastore,
        query,
        function _afterTalkingToAdapter(err, findInbound) {
          if (err) {
          ...
          // the findInbound sent back by the adapter turns out to be something other
          ...
          // ```
          // !_.isNumber(findInbound) || findInbound === Infinity || findInbound === -Infinity || _.isNaN(findInbound)
          // ````
          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

          return done(undefined, findInbound);
        }
      ); //</adapter.findInbound()>
    },
