...
module.exports = function archive(/* criteria, explicitCbMaybe, metaContainer */) {
  ...
  return parley(
    function (done) {
          ...
          flaverr(
            {
              name: 'UsageError',
              message:
                'Since the `archiveModelIdentity` setting was explicitly disabled, .archive() cannot be used.',
            },
            omen
          )
        );
      ...
      WLModel.find(
        query.criteria,
        function _afterFinding(err, foundRecords) {
          if (err) {
            return done(err);
          }
          ...
          Archive.createEach(
            archives,
            function _afterCreatingEach(err) {
              if (err) {
                return done(err);
              }
