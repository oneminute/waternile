...
module.exports = function destroyOne(criteria, explicitCbMaybe, metaContainer) {
  ...
  return parley(
    function (done) {
      ...
      var modifiedCriteriaForCount = _.omit(query.criteria, [
        'select',
        'omit',
        'limit',
        'skip',
      ...
      WLModel.count(
        modifiedCriteriaForCount,
        function _afterCounting(err, total) {
          if (err) {
            return done(err);
          }
          ...
          var modifiedCriteriaForDestroy = _.omit(query.criteria, [
            'select',
            'omit',
            'limit',
            'skip',
          ...
          WLModel.destroy(
            modifiedCriteriaForDestroy,
            function _afterDestroying(err, affectedRecords) {
              if (err) {
                return done(err);
              }
