...
var MetaModel = (module.exports = function MetaModel(orm, adapterWrapper) {
  ...
  if (_.isUndefined(this.attributes)) {
    ...
    if (!_.isObject(this.attributes)) {
      throw new Error(
        'Consistency violation: When instantiating this new instance of MetaModel, it became clear (within the constructor) that `this.attributes` was already set, and not a dictionary: ' +
          util.inspect(this.attributes, { depth: 5 }) +
          ''
      );
    } else {
...
//Added by Gaitho
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// A function for defining variables to be used in AQL
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

_.extend(MetaModel.prototype, {
  let: function let(variables) {
    if (_.isUndefined(variables)) {
      this._let = {};
    } else {
      if (!_.isObject(variables)) {
        throw new Error('Let attributes must be a dictionary');
      } else {
        this._let = { ...variables };
      }
    }
  ...
  //Added to support Graphs. by Gaitho.
  createEdge: require('./methods/create-edge'),
  findOutbound: require('./methods/find-outbound'),
  findInbound: require('./methods/find-inbound'),
  findWithCount: require('./methods/find-with-count'),
  findAny: require('./methods/find-any'),
  upsert: require('./methods/upsert'),
  findNear: require('./methods/find-near'),
  aggregate: require('./methods/aggregate'),
  normalize: require('./methods/normalize'),
  normalizeEach: require('./methods/normalize-each'),
...
MetaModel.extend = function (protoProps, staticProps) {
  ...
  if (protoProps && _.has(protoProps, 'constructor')) {
    throw new Error(
      'Consistency violation: The first argument (`protoProps`) provided to Waterline.Model.extend() should never have a `constructor` property. (This kind of usage is no longer supported.)'
    );
  }
  ...
  if (!_.isUndefined(staticProps)) {
    throw new Error(
      'Consistency violation: Unrecognized extra argument provided to Waterline.Model.extend() (`staticProps` is no longer supported.)'
    );
  }
  ...
  var newConstructor = function () {
    return thisConstructor.apply(this, arguments);
  };
